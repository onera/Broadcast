...
!
!  Differentiation of flux_num_dnc5_polar_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs
! with!StaticTaping):
...
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of flux_num_dnc5_polar_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs w
!ith!StaticTaping):
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC5_POLAR_2D_D_D(residu, residud, residudd, w, wd0&
...
!
      ...
      ELSE
        ...
        abs0 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
      END IF
      x8dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
      x8d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
      x8d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
      temp3 = abs0*abs14d/abs14
      temp2 = (abs0d-temp3)/abs14
      k_sensor1dd = (abs0dd-(abs14d*abs0d0+abs0*abs14dd-temp3*abs14d0)/&
&       abs14-temp2*abs14d0)/abs14
      k_sensor1d = temp2
      k_sensor1d0 = (abs0d0-abs0*abs14d0/abs14)/abs14
      ...
      ELSE
        ...
        abs1 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
      END IF
      x9dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
      x9d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
      x9d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
      ...
      ELSE
        ...
        abs15 = -x9
      END IF
      temp3 = abs1*abs15d/abs15
      temp2 = (abs1d-temp3)/abs15
      k_sensor2dd = (abs1dd-(abs15d*abs1d0+abs1*abs15dd-temp3*abs15d0)/&
&       abs15-temp2*abs15d0)/abs15
      k_sensor2d = temp2
      k_sensor2d0 = (abs1d0-abs1*abs15d0/abs15)/abs15
...
!                 k_sensor2*ducros2*dxm2)
      temp3 = max8*max1d + max1*max8d
      coefdd = temp3*max15d0 + max15*(max1d*max8d0+max8*max1dd+max8d*&
&       max1d0+max1*max8dd) + max15d*(max8*max1d0+max1*max8d0) + max1*&
&       max8*max15dd
      coefd = max15*temp3 + max1*max8*max15d
      coefd0 = max15*(max8*max1d0+max1*max8d0) + max1*max8*max15d0
...
!
      ...
      ELSE
        ...
        abs2 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
      END IF
      x10dd = pdd(i, j-1) + two*pdd(i, j) + pdd(i, j+1)
      x10d = pd(i, j-1) + two*pd(i, j) + pd(i, j+1)
      x10d0 = pd0(i, j-1) + two*pd0(i, j) + pd0(i, j+1)
...
!
! 2nd direction
      temp3 = abs2*abs16d/abs16
      temp2 = (abs2d-temp3)/abs16
      k_sensor1dd = (abs2dd-(abs16d*abs2d0+abs2*abs16dd-temp3*abs16d0)/&
&       abs16-temp2*abs16d0)/abs16
      k_sensor1d = temp2
      k_sensor1d0 = (abs2d0-abs2*abs16d0/abs16)/abs16
      ...
      ELSE
        ...
        abs3 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
      END IF
      x11dd = pdd(i, j-2) + two*pdd(i, j-1) + pdd(i, j)
      x11d = pd(i, j-2) + two*pd(i, j-1) + pd(i, j)
      x11d0 = pd0(i, j-2) + two*pd0(i, j-1) + pd0(i, j)
      ...
      ELSE
        ...
        abs17 = -x11
      END IF
      temp3 = abs3*abs17d/abs17
      temp2 = (abs3d-temp3)/abs17
      k_sensor2dd = (abs3dd-(abs17d*abs3d0+abs3*abs17dd-temp3*abs17d0)/&
&       abs17-temp2*abs17d0)/abs17
      k_sensor2d = temp2
      k_sensor2d0 = (abs3d0-abs3*abs17d0/abs17)/abs17
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
      temp3 = max9*max2d + max2*max9d
      coefdd = temp3*max16d0 + max16*(max2d*max9d0+max9*max2dd+max9d*&
&       max2d0+max2*max9dd) + max16d*(max9*max2d0+max2*max9d0) + max2*&
&       max9*max16dd
      coefd = max16*temp3 + max2*max9*max16d
      coefd0 = max16*(max9*max2d0+max2*max9d0) + max2*max9*max16d0
...
&     , j, 1)+fdd(i-2, j, 1))+c3*(fdd(i+2, j, 1)+fdd(i-3, j, 1))) + ny(i&
&     , j, 1)*(c1*(gdd(i, j, 1)+gdd(i-1, j, 1))+c2*(gdd(i+1, j, 1)+gdd(i&
&     -2, j, 1))+c3*(gdd(i+2, j, 1)+gdd(i-3, j, 1)))
    fxrou1dd = nx(i, j, 1)*(c1*(fdd(i, j, 2)+fdd(i-1, j, 2))+c2*(fdd(i+1&
&     , j, 2)+fdd(i-2, j, 2))+c3*(fdd(i+2, j, 2)+fdd(i-3, j, 2))) + ny(i&
&     , j, 1)*(c1*(gdd(i, j, 2)+gdd(i-1, j, 2))+c2*(gdd(i+1, j, 2)+gdd(i&
&     -2, j, 2))+c3*(gdd(i+2, j, 2)+gdd(i-3, j, 2)))
    fxrov1dd = nx(i, j, 1)*(c1*(fdd(i, j, 3)+fdd(i-1, j, 3))+c2*(fdd(i+1&
&     , j, 3)+fdd(i-2, j, 3))+c3*(fdd(i+2, j, 3)+fdd(i-3, j, 3))) + ny(i&
&     , j, 1)*(c1*(gdd(i, j, 3)+gdd(i-1, j, 3))+c2*(gdd(i+1, j, 3)+gdd(i&
&     -2, j, 3))+c3*(gdd(i+2, j, 3)+gdd(i-3, j, 3)))
    fxrow1dd = nx(i, j, 1)*(c1*(fdd(i, j, 4)+fdd(i-1, j, 4))+c2*(fdd(i+1&
&     , j, 4)+fdd(i-2, j, 4))+c3*(fdd(i+2, j, 4)+fdd(i-3, j, 4))) + ny(i&
&     , j, 1)*(c1*(gdd(i, j, 4)+gdd(i-1, j, 4))+c2*(gdd(i+1, j, 4)+gdd(i&
&     -2, j, 4))+c3*(gdd(i+2, j, 4)+gdd(i-3, j, 4)))
    fxroe1dd = nx(i, j, 1)*(c1*(fdd(i, j, 5)+fdd(i-1, j, 5))+c2*(fdd(i+1&
&     , j, 5)+fdd(i-2, j, 5))+c3*(fdd(i+2, j, 5)+fdd(i-3, j, 5))) + ny(i&
&     , j, 1)*(c1*(gdd(i, j, 5)+gdd(i-1, j, 5))+c2*(gdd(i+1, j, 5)+gdd(i&
&     -2, j, 5))+c3*(gdd(i+2, j, 5)+gdd(i-3, j, 5)))
! Saunier Thesis
    fxro2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 1)+c5d1*fdd(i, 2, 1)+c5d2*fdd(&
&     i, 3, 1)+c5d3*fdd(i, 4, 1)+c5d4*fdd(i, 5, 1)) + ny(i, j, 2)*(c5d0*&
&     gdd(i, 1, 1)+c5d1*gdd(i, 2, 1)+c5d2*gdd(i, 3, 1)+c5d3*gdd(i, 4, 1)&
&     +c5d4*gdd(i, 5, 1))
    fxrou2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 2)+c5d1*fdd(i, 2, 2)+c5d2*fdd&
&     (i, 3, 2)+c5d3*fdd(i, 4, 2)+c5d4*fdd(i, 5, 2)) + ny(i, j, 2)*(c5d0&
&     *gdd(i, 1, 2)+c5d1*gdd(i, 2, 2)+c5d2*gdd(i, 3, 2)+c5d3*gdd(i, 4, 2&
&     )+c5d4*gdd(i, 5, 2))
    fxrov2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 3)+c5d1*fdd(i, 2, 3)+c5d2*fdd&
&     (i, 3, 3)+c5d3*fdd(i, 4, 3)+c5d4*fdd(i, 5, 3)) + ny(i, j, 2)*(c5d0&
&     *gdd(i, 1, 3)+c5d1*gdd(i, 2, 3)+c5d2*gdd(i, 3, 3)+c5d3*gdd(i, 4, 3&
&     )+c5d4*gdd(i, 5, 3))
    fxrow2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 4)+c5d1*fdd(i, 2, 4)+c5d2*fdd&
&     (i, 3, 4)+c5d3*fdd(i, 4, 4)+c5d4*fdd(i, 5, 4)) + ny(i, j, 2)*(c5d0&
&     *gdd(i, 1, 4)+c5d1*gdd(i, 2, 4)+c5d2*gdd(i, 3, 4)+c5d3*gdd(i, 4, 4&
&     )+c5d4*gdd(i, 5, 4))
    fxroe2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 5)+c5d1*fdd(i, 2, 5)+c5d2*fdd&
&     (i, 3, 5)+c5d3*fdd(i, 4, 5)+c5d4*fdd(i, 5, 5)) + ny(i, j, 2)*(c5d0&
&     *gdd(i, 1, 5)+c5d1*gdd(i, 2, 5)+c5d2*gdd(i, 3, 5)+c5d3*gdd(i, 4, 5&
&     )+c5d4*gdd(i, 5, 5))
    predro1d = d2*wd(i-2, j, 1) - d3*wd(i-3, j, 1) - d1*wd(i-1, j, 1) + &
&     d1*wd(i, j, 1) - d2*wd(i+1, j, 1) + d3*wd(i+2, j, 1)
    predro1d0 = d2*wd0(i-2, j, 1) - d3*wd0(i-3, j, 1) - d1*wd0(i-1, j, 1&
&     ) + d1*wd0(i, j, 1) - d2*wd0(i+1, j, 1) + d3*wd0(i+2, j, 1)
    predro1 = -(d3*w(i-3, j, 1)) + d2*w(i-2, j, 1) - d1*w(i-1, j, 1) + &
&     d1*w(i, j, 1) - d2*w(i+1, j, 1) + d3*w(i+2, j, 1)
    predrou1d = d2*wd(i-2, j, 2) - d3*wd(i-3, j, 2) - d1*wd(i-1, j, 2) +&
&     d1*wd(i, j, 2) - d2*wd(i+1, j, 2) + d3*wd(i+2, j, 2)
...
&     d1*w(i, j, 5) - d2*w(i+1, j, 5) + d3*w(i+2, j, 5)
    predro2d = d2*wd(i, j-2, 1) - d3*wd(i, j-3, 1) - d1*wd(i, j-1, 1) + &
&     d1*wd(i, j, 1) - d2*wd(i, j+1, 1) + d3*wd(i, j+2, 1)
    predro2d0 = d2*wd0(i, j-2, 1) - d3*wd0(i, j-3, 1) - d1*wd0(i, j-1, 1&
&     ) + d1*wd0(i, j, 1) - d2*wd0(i, j+1, 1) + d3*wd0(i, j+2, 1)
    predro2 = -(d3*w(i, j-3, 1)) + d2*w(i, j-2, 1) - d1*w(i, j-1, 1) + &
&     d1*w(i, j, 1) - d2*w(i, j+1, 1) + d3*w(i, j+2, 1)
    predrou2d = d2*wd(i, j-2, 2) - d3*wd(i, j-3, 2) - d1*wd(i, j-1, 2) +&
&     d1*wd(i, j, 2) - d2*wd(i, j+1, 2) + d3*wd(i, j+2, 2)
...
!
!1st direction
    temp3 = abs4*abs18d/abs18
    temp2 = (abs4d-temp3)/abs18
    k_sensor1dd = (abs4dd-(abs18d*abs4d0+abs4*abs18dd-temp3*abs18d0)/&
&     abs18-temp2*abs18d0)/abs18
    k_sensor1d = temp2
    k_sensor1d0 = (abs4d0-abs4*abs18d0/abs18)/abs18
    ...
    ELSE
      ...
      abs19 = -x13
    END IF
    temp3 = abs5*abs19d/abs19
    temp2 = (abs5d-temp3)/abs19
    k_sensor2dd = (abs5dd-(abs19d*abs5d0+abs5*abs19dd-temp3*abs19d0)/&
&     abs19-temp2*abs19d0)/abs19
    k_sensor2d = temp2
    k_sensor2d0 = (abs5d0-abs5*abs19d0/abs19)/abs19
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max10*max3d + max3*max10d
    coefdd = temp3*max17d0 + max17*(max3d*max10d0+max10*max3dd+max10d*&
&     max3d0+max3*max10dd) + max17d*(max10*max3d0+max3*max10d0) + max3*&
&     max10*max17dd
    coefd = max17*temp3 + max3*max10*max17d
    coefd0 = max17*(max10*max3d0+max3*max10d0) + max3*max10*max17d0
...
!
! 2nd direction
    temp3 = abs6*abs20d/abs20
    temp2 = (abs6d-temp3)/abs20
    k_sensor1dd = (abs6dd-(abs20d*abs6d0+abs6*abs20dd-temp3*abs20d0)/&
&     abs20-temp2*abs20d0)/abs20
    k_sensor1d = temp2
    k_sensor1d0 = (abs6d0-abs6*abs20d0/abs20)/abs20
    ...
    ELSE
      ...
      abs21 = -x15
    END IF
    temp3 = abs7*abs21d/abs21
    temp2 = (abs7d-temp3)/abs21
    k_sensor2dd = (abs7dd-(abs21d*abs7d0+abs7*abs21dd-temp3*abs21d0)/&
&     abs21-temp2*abs21d0)/abs21
    k_sensor2d = temp2
    k_sensor2d0 = (abs7d0-abs7*abs21d0/abs21)/abs21
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max11*max4d + max4*max11d
    coefdd = temp3*max18d0 + max18*(max4d*max11d0+max11*max4dd+max11d*&
&     max4d0+max4*max11dd) + max18d*(max11*max4d0+max4*max11d0) + max4*&
&     max11*max18dd
    coefd = max18*temp3 + max4*max11*max18d
    coefd0 = max18*(max11*max4d0+max4*max11d0) + max4*max11*max18d0
...
&     i, 3, 1)+c3d3*fdd(i, 4, 1)+c3d4*fdd(i, 5, 1)) + ny(i, j, 2)*(c3d0*&
&     gdd(i, 1, 1)+c3d1*gdd(i, 2, 1)+c3d2*gdd(i, 3, 1)+c3d3*gdd(i, 4, 1)&
&     +c3d4*gdd(i, 5, 1))
    fxrou2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 2)+c3d1*fdd(i, 2, 2)+c3d2*fdd&
&     (i, 3, 2)+c3d3*fdd(i, 4, 2)+c3d4*fdd(i, 5, 2)) + ny(i, j, 2)*(c3d0&
&     *gdd(i, 1, 2)+c3d1*gdd(i, 2, 2)+c3d2*gdd(i, 3, 2)+c3d3*gdd(i, 4, 2&
&     )+c3d4*gdd(i, 5, 2))
    fxrov2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 3)+c3d1*fdd(i, 2, 3)+c3d2*fdd&
&     (i, 3, 3)+c3d3*fdd(i, 4, 3)+c3d4*fdd(i, 5, 3)) + ny(i, j, 2)*(c3d0&
&     *gdd(i, 1, 3)+c3d1*gdd(i, 2, 3)+c3d2*gdd(i, 3, 3)+c3d3*gdd(i, 4, 3&
&     )+c3d4*gdd(i, 5, 3))
    fxrow2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 4)+c3d1*fdd(i, 2, 4)+c3d2*fdd&
&     (i, 3, 4)+c3d3*fdd(i, 4, 4)+c3d4*fdd(i, 5, 4)) + ny(i, j, 2)*(c3d0&
&     *gdd(i, 1, 4)+c3d1*gdd(i, 2, 4)+c3d2*gdd(i, 3, 4)+c3d3*gdd(i, 4, 4&
&     )+c3d4*gdd(i, 5, 4))
    fxroe2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 5)+c3d1*fdd(i, 2, 5)+c3d2*fdd&
&     (i, 3, 5)+c3d3*fdd(i, 4, 5)+c3d4*fdd(i, 5, 5)) + ny(i, j, 2)*(c3d0&
&     *gdd(i, 1, 5)+c3d1*gdd(i, 2, 5)+c3d2*gdd(i, 3, 5)+c3d3*gdd(i, 4, 5&
&     )+c3d4*gdd(i, 5, 5))
    predro1d = d2*wd(i-2, j, 1) - d3*wd(i-3, j, 1) - d1*wd(i-1, j, 1) + &
&     d1*wd(i, j, 1) - d2*wd(i+1, j, 1) + d3*wd(i+2, j, 1)
    predro1d0 = d2*wd0(i-2, j, 1) - d3*wd0(i-3, j, 1) - d1*wd0(i-1, j, 1&
...
!
!1st direction
    temp3 = abs8*abs22d/abs22
    temp2 = (abs8d-temp3)/abs22
    k_sensor1dd = (abs8dd-(abs22d*abs8d0+abs8*abs22dd-temp3*abs22d0)/&
&     abs22-temp2*abs22d0)/abs22
    k_sensor1d = temp2
    k_sensor1d0 = (abs8d0-abs8*abs22d0/abs22)/abs22
    ...
    ELSE
      ...
      abs23 = -x17
    END IF
    temp3 = abs9*abs23d/abs23
    temp2 = (abs9d-temp3)/abs23
    k_sensor2dd = (abs9dd-(abs23d*abs9d0+abs9*abs23dd-temp3*abs23d0)/&
&     abs23-temp2*abs23d0)/abs23
    k_sensor2d = temp2
    k_sensor2d0 = (abs9d0-abs9*abs23d0/abs23)/abs23
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max12*max5d + max5*max12d
    coefdd = temp3*max19d0 + max19*(max5d*max12d0+max12*max5dd+max12d*&
&     max5d0+max5*max12dd) + max19d*(max12*max5d0+max5*max12d0) + max5*&
&     max12*max19dd
    coefd = max19*temp3 + max5*max12*max19d
    coefd0 = max19*(max12*max5d0+max5*max12d0) + max5*max12*max19d0
...
!
! 2nd direction
    temp3 = abs10*abs24d/abs24
    temp2 = (abs10d-temp3)/abs24
    k_sensor1dd = (abs10dd-(abs24d*abs10d0+abs10*abs24dd-temp3*abs24d0)/&
&     abs24-temp2*abs24d0)/abs24
    k_sensor1d = temp2
    k_sensor1d0 = (abs10d0-abs10*abs24d0/abs24)/abs24
    ...
    ELSE
      ...
      abs25 = -x19
    END IF
    temp3 = abs11*abs25d/abs25
    temp2 = (abs11d-temp3)/abs25
    k_sensor2dd = (abs11dd-(abs25d*abs11d0+abs11*abs25dd-temp3*abs25d0)/&
&     abs25-temp2*abs25d0)/abs25
    k_sensor2d = temp2
    k_sensor2d0 = (abs11d0-abs11*abs25d0/abs25)/abs25
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max13*max6d + max6*max13d
    coefdd = temp3*max20d0 + max20*(max6d*max13d0+max13*max6dd+max13d*&
&     max6d0+max6*max13dd) + max20d*(max13*max6d0+max6*max13d0) + max6*&
&     max13*max20dd
    coefd = max20*temp3 + max6*max13*max20d
    coefd0 = max20*(max13*max6d0+max6*max13d0) + max6*max13*max20d0
...
!
!1st direction
    temp3 = abs12*abs26d/abs26
    temp2 = (abs12d-temp3)/abs26
    k_sensor1dd = (abs12dd-(abs26d*abs12d0+abs12*abs26dd-temp3*abs26d0)/&
&     abs26-temp2*abs26d0)/abs26
    k_sensor1d = temp2
    k_sensor1d0 = (abs12d0-abs12*abs26d0/abs26)/abs26
    ...
    ELSE
      ...
      abs27 = -x21
    END IF
    temp3 = abs13*abs27d/abs27
    temp2 = (abs13d-temp3)/abs27
    k_sensor2dd = (abs13dd-(abs27d*abs13d0+abs13*abs27dd-temp3*abs27d0)/&
&     abs27-temp2*abs27d0)/abs27
    k_sensor2d = temp2
    k_sensor2d0 = (abs13d0-abs13*abs27d0/abs27)/abs27
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max14*max7d + max7*max14d
    coefdd = temp3*max21d0 + max21*(max7d*max14d0+max14*max7dd+max14d*&
&     max7d0+max7*max14dd) + max21d*(max14*max7d0+max7*max14d0) + max7*&
&     max14*max21dd
    coefd = max21*temp3 + max7*max14*max21d
    coefd0 = max21*(max14*max7d0+max7*max14d0) + max7*max14*max21d0
...
&       + hndd(i, j, 5, 2) - hndd(i, j+1, 5, 2)
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC5_POLAR_2D_D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
