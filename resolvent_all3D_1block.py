...
if __name__ == '__main__':
    ...
    if rank ==0:
        for k in range(len(eigenvalue)):
            ...
            filename = out_dir + "/response_atcenter_eig_om{:.2f}_be{:.2f}_n{:d}_real.dat".format(freq, wave, k)
            ...
            filename = out_dir + "/response_atcenter_eig_om{:.2f}_be{:.2f}_n{:d}_imag.dat".format(freq, wave, k)
            __writestate_center_gh(filename, im, jm, w_responsei, x, y)
            # comm.Barrier()
            f_opt = _np.reshape( _np.real(_np.array(eigenvector_forcing[k])), (im,jm,5))
            filename = out_dir + "/forcing_atcenter_eig_om{:.2f}_be{:.2f}_n{:d}_real.dat".format(freq, wave, k)
            __writestate_center_gh(filename, im, jm, f_opt, x, y)
            # comm.Barrier()
            f_opti = _np.reshape( _np.imag(_np.array(eigenvector_forcing[k])), (im,jm,5))
            filename = out_dir + "/forcing_atcenter_eig_om{:.2f}_be{:.2f}_n{:d}_imag.dat".format(freq, wave, k)
