...
SUBROUTINE BC_SUPANDSUBINLET_FORHESSIAN_2D_D(wout, woutd, w, wd, loc, &
...
! l-state
        ...
        udd = rom1*woutd(i-de*i0, j-de*j0, 2) + temp*rom1d
        ud = temp*rom1
        temp = wout(i-de*i0, j-de*j0, 3)
        vdd = rom1*woutd(i-de*i0, j-de*j0, 3) + temp*rom1d
        vd = temp*rom1
        temp = wout(i-de*i0, j-de*j0, 4)
        wdid = rom1*woutd(i-de*i0, j-de*j0, 4) + temp*rom1d
        wdi = temp*rom1
        temp = ud*ud + vd*vd + wdi*wdi
        pdd = gam1*(woutd(i-de*i0, j-de*j0, 5)-half*(temp*rodd+rod*(2*ud&
&         *udd+2*vd*vdd+2*wdi*wdid)))
        ...
        hld = rom1*(woutd(i-de*i0, j-de*j0, 5)+pdd) + temp*rom1d
        hl = temp*rom1
! r-state
        ...
        usd = rosm1*woutd(i-da*i0, j-da*j0, 2) + temp*rosm1d
        us = temp*rosm1
        temp = wout(i-da*i0, j-da*j0, 3)
        vsd = rosm1*woutd(i-da*i0, j-da*j0, 3) + temp*rosm1d
        vs = temp*rosm1
        temp = wout(i-da*i0, j-da*j0, 4)
        wsd = rosm1*woutd(i-da*i0, j-da*j0, 4) + temp*rosm1d
        ws = temp*rosm1
        temp = us*us + vs*vs + ws*ws
        psd = gam1*(woutd(i-da*i0, j-da*j0, 5)-half*(temp*rosd+ros*(2*us&
&         *usd+2*vs*vsd+2*ws*wsd)))
        ...
        hrd = rosm1*(woutd(i-da*i0, j-da*j0, 5)+psd) + temp*rosm1d
        hr = temp*rosm1
        temp = SQRT(ros*rom1)
        ...
        wwd = oneonrplusone*(r*wsd+ws*rd+wdid) + (ws*r+wdi)*&
&         oneonrplusoned
...
&         roc0m1*roc0m1d + rodd
        ...
        woutd(i-de*i0, j-de*j0, 5) = roed + half*(2*rou*roud+2*rov*rovd-&
&         temp*rod0)/ro
        wout(i-de*i0, j-de*j0, 5) = roe + half*temp
      END IF
    END DO
  END DO
END SUBROUTINE BC_SUPANDSUBINLET_FORHESSIAN_2D_D
