...
!
!  Differentiation of bc_extrapolate_o5_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs wit
!h!StaticTaping):
!   variations   of useful results: w
!   with respect to varying inputs: w
!   RW status of diff variables: w:in-out
SUBROUTINE BC_EXTRAPOLATE_O5_2D_D(w, wd, loc, interf, im, jm, gh, em)
  IMPLICIT NONE
...
!   write(200,*) loc, 'jmax = ', interf(2,2)
  ...
  DO l=lmin,lmax
    ...
    DO de=1,gh
      ...
      d5 = de - 5
!ordre 5
      wd(i-i0*de, j-j0*de, :) = five*wd(i-i0*d1, j-j0*d1, :) - ten*wd(i-&
&       i0*d2, j-j0*d2, :) + ten*wd(i-i0*d3, j-j0*d3, :) + wd(i-i0*d5, j&
&       -j0*d5, :) - five*wd(i-i0*d4, j-j0*d4, :)
    END DO
  END DO
END SUBROUTINE BC_EXTRAPOLATE_O5_2D_D
