...
!
!  Differentiation of bc_mirror_anti_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs with!S
!taticTaping):
...
! =============================================================================
SUBROUTINE BC_MIRROR_ANTI_2D_B(w, wb, loc, interf, nx, ny, gh, im, jm)
  IMPLICIT NONE
...
! highis used to select the last normal when the face is of type *hi
    ...
    DO de=1,gh
      da = de - 1
      vi = zero
      vj = SQRT(w(i+da*i0, j+da*j0, 2)*w(i+da*i0, j+da*j0, 2)*nynorm*&
&       nynorm + w(i+da*i0, j+da*j0, 3)*w(i+da*i0, j+da*j0, 3)*nxnorm*&
&       nxnorm)
      tmp = -w(i+da*i0, j+da*j0, 1)
      CALL PUSHREAL8(w(i-de*i0, j-de*j0, 1))
      w(i-de*i0, j-de*j0, 1) = tmp
...
! w(i-de*i0,j-de*j0,3) = ZERO
      ...
      tmp0 = -w(i+da*i0, j+da*j0, 4)
      CALL PUSHREAL8(w(i-de*i0, j-de*j0, 4))
      w(i-de*i0, j-de*j0, 4) = tmp0
      tmp1 = -w(i+da*i0, j+da*j0, 5)
      CALL PUSHREAL8(w(i-de*i0, j-de*j0, 5))
      w(i-de*i0, j-de*j0, 5) = tmp1
    END DO
    DO de=gh,1,-1
      da = de - 1
      CALL POPREAL8(w(i-de*i0, j-de*j0, 5))
      tmpb1 = wb(i-de*i0, j-de*j0, 5)
      wb(i-de*i0, j-de*j0, 5) = 0.0_8
      wb(i+da*i0, j+da*j0, 5) = wb(i+da*i0, j+da*j0, 5) - tmpb1
      CALL POPREAL8(w(i-de*i0, j-de*j0, 4))
      tmpb0 = wb(i-de*i0, j-de*j0, 4)
      wb(i-de*i0, j-de*j0, 4) = 0.0_8
      wb(i+da*i0, j+da*j0, 4) = wb(i+da*i0, j+da*j0, 4) - tmpb0
      CALL POPREAL8(w(i-de*i0, j-de*j0, 3))
      vjb = nxnorm*wb(i-de*i0, j-de*j0, 3) + nynorm*wb(i-de*i0, j-de*j0&
&       , 2)
      wb(i-de*i0, j-de*j0, 3) = 0.0_8
      CALL POPREAL8(w(i-de*i0, j-de*j0, 2))
      wb(i-de*i0, j-de*j0, 2) = 0.0_8
      CALL POPREAL8(w(i-de*i0, j-de*j0, 1))
      tmpb = wb(i-de*i0, j-de*j0, 1)
      wb(i-de*i0, j-de*j0, 1) = 0.0_8
      wb(i+da*i0, j+da*j0, 1) = wb(i+da*i0, j+da*j0, 1) - tmpb
      temp = w(i+da*i0, j+da*j0, 3)
      temp0 = w(i+da*i0, j+da*j0, 2)
      IF (nynorm**2*temp0**2 + nxnorm**2*temp**2 .EQ. 0.0) THEN
        tempb = 0.0_8
      ELSE
        tempb = vjb/(2.0*SQRT(nynorm**2*temp0**2+nxnorm**2*temp**2))
      END IF
      wb(i+da*i0, j+da*j0, 2) = wb(i+da*i0, j+da*j0, 2) + 2*temp0*nynorm&
&       **2*tempb
      wb(i+da*i0, j+da*j0, 3) = wb(i+da*i0, j+da*j0, 3) + 2*temp*nxnorm&
&       **2*tempb
    END DO
  END DO
END SUBROUTINE BC_MIRROR_ANTI_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
