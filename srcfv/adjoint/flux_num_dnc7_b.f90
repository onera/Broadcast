...
!
!  Differentiation of flux_num_dnc7_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs with!St
!aticTaping):
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC7_2D_B(residu, residub, w, wb, x0, y0, nx, ny, xc&
...
&     *dissro1b
    ...
    IF (ad_branch79 .EQ. 0) eps2b = eps2b - 12.d0*eps4b
    coefb = k2*eps2b
    max9b = max18*max27*coefb
    max18b = max9*max27*coefb
    max27b = max9*max18*coefb
...
&     1.d-15))
    ...
    abs17b = k_sensor2b/abs35
    abs35b = -(abs17*k_sensor2b/abs35**2)
    IF (ad_branch75 .EQ. 0) THEN
      x27b = abs35b
    ELSE
      x27b = -abs35b
    END IF
    pb(i-2, j) = pb(i-2, j) + x27b
    pb(i-1, j) = pb(i-1, j) + two*x27b
    pb(i, j) = pb(i, j) + x27b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs17b
    END IF
    abs16b = k_sensor1b/abs34
    abs34b = -(abs16*k_sensor1b/abs34**2)
    IF (ad_branch73 .EQ. 0) THEN
      x26b = abs34b
    ELSE
      x26b = -abs34b
    END IF
    pb(i-1, j) = pb(i-1, j) + x26b
...
&     *dissro2b
    ...
    END IF
    coefb = k2*eps2b
    max8b = max17*max26*coefb
    max17b = max8*max26*coefb
    max26b = max8*max17*coefb
...
&     1.d-15))
    ...
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    divu = gradu(i-1, j, 1) + gradv(i-1, j, 2)
    abs15b = k_sensor2b/abs33
    abs33b = -(abs15*k_sensor2b/abs33**2)
    IF (ad_branch66 .EQ. 0) THEN
      x25b = abs33b
    ELSE
      x25b = -abs33b
    END IF
    pb(i, j-2) = pb(i, j-2) + x25b
    pb(i, j-1) = pb(i, j-1) + two*x25b
    pb(i, j) = pb(i, j) + x25b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs15b
    END IF
    abs14b = k_sensor1b/abs32
    abs32b = -(abs14*k_sensor1b/abs32**2)
    IF (ad_branch64 .EQ. 0) THEN
      x24b = abs32b
    ELSE
      x24b = -abs32b
    END IF
    pb(i, j-1) = pb(i, j-1) + x24b
    pb(i, j) = pb(i, j) + two*x24b
    pb(i, j+1) = pb(i, j+1) + x24b
    IF (ad_branch63 .EQ. 0) THEN
...
&     *dissro1b
    ...
    coefb = k2*eps2b
    max7b = max16*max25*coefb
    max16b = max7*max25*coefb
    max25b = max7*max16*coefb
...
&     1.d-15))
    ...
    gradub(i, j, 1) = gradub(i, j, 1) + divub
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    abs13b = k_sensor2b/abs31
    abs31b = -(abs13*k_sensor2b/abs31**2)
    IF (ad_branch57 .EQ. 0) THEN
      x23b = abs31b
    ELSE
      x23b = -abs31b
    END IF
    pb(i-2, j) = pb(i-2, j) + x23b
    pb(i-1, j) = pb(i-1, j) + two*x23b
    pb(i, j) = pb(i, j) + x23b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs13b
    END IF
    abs12b = k_sensor1b/abs30
    abs30b = -(abs12*k_sensor1b/abs30**2)
    IF (ad_branch55 .EQ. 0) THEN
      x22b = abs30b
    ELSE
      x22b = -abs30b
    END IF
    pb(i-1, j) = pb(i-1, j) + x22b
...
&     *dissro2b
    ...
    END IF
    coefb = k2*eps2b
    max6b = max15*max24*coefb
    max15b = max6*max24*coefb
    max24b = max6*max15*coefb
...
&     1.d-15))
    ...
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    divu = gradu(i-1, j, 1) + gradv(i-1, j, 2)
    abs11b = k_sensor2b/abs29
    abs29b = -(abs11*k_sensor2b/abs29**2)
    IF (ad_branch48 .EQ. 0) THEN
      x21b = abs29b
    ELSE
      x21b = -abs29b
    END IF
    pb(i, j-2) = pb(i, j-2) + x21b
    pb(i, j-1) = pb(i, j-1) + two*x21b
    pb(i, j) = pb(i, j) + x21b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs11b
    END IF
    abs10b = k_sensor1b/abs28
    abs28b = -(abs10*k_sensor1b/abs28**2)
    IF (ad_branch46 .EQ. 0) THEN
      x20b = abs28b
    ELSE
      x20b = -abs28b
    END IF
    pb(i, j-1) = pb(i, j-1) + x20b
...
&     *dissro1b
    ...
    coefb = k2*eps2b
    max5b = max14*max23*coefb
    max14b = max5*max23*coefb
    max23b = max5*max14*coefb
...
&     1.d-15))
    ...
    gradub(i, j, 1) = gradub(i, j, 1) + divub
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    abs9b = k_sensor2b/abs27
    abs27b = -(abs9*k_sensor2b/abs27**2)
    IF (ad_branch39 .EQ. 0) THEN
      x19b = abs27b
    ELSE
      x19b = -abs27b
    END IF
    pb(i-2, j) = pb(i-2, j) + x19b
    pb(i-1, j) = pb(i-1, j) + two*x19b
    pb(i, j) = pb(i, j) + x19b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs9b
    END IF
    abs8b = k_sensor1b/abs26
    abs26b = -(abs8*k_sensor1b/abs26**2)
    IF (ad_branch37 .EQ. 0) THEN
      x18b = abs26b
    ELSE
      x18b = -abs26b
    END IF
    pb(i-1, j) = pb(i-1, j) + x18b
...
&     *dissro2b
    ...
    END IF
    coefb = k2*eps2b
    max4b = max13*max22*coefb
    max13b = max4*max22*coefb
    max22b = max4*max13*coefb
...
&     1.d-15))
    ...
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    divu = gradu(i-1, j, 1) + gradv(i-1, j, 2)
    abs7b = k_sensor2b/abs25
    abs25b = -(abs7*k_sensor2b/abs25**2)
    IF (ad_branch30 .EQ. 0) THEN
      x17b = abs25b
    ELSE
      x17b = -abs25b
    END IF
    pb(i, j-2) = pb(i, j-2) + x17b
    pb(i, j-1) = pb(i, j-1) + two*x17b
    pb(i, j) = pb(i, j) + x17b
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs7b
    END IF
    abs6b = k_sensor1b/abs24
    abs24b = -(abs6*k_sensor1b/abs24**2)
    IF (ad_branch28 .EQ. 0) THEN
      x16b = abs24b
    ELSE
      x16b = -abs24b
    END IF
    pb(i, j-1) = pb(i, j-1) + x16b
...
&     *dissro1b
    ...
    coefb = k2*eps2b
    max3b = max12*max21*coefb
    max12b = max3*max21*coefb
    max21b = max3*max12*coefb
...
&     1.d-15))
    ...
    gradub(i, j, 1) = gradub(i, j, 1) + divub
    gradvb(i, j, 2) = gradvb(i, j, 2) + divub
    abs5b = k_sensor2b/abs23
    abs23b = -(abs5*k_sensor2b/abs23**2)
    ...
    ELSE
      ...
      pb(i, j) = pb(i, j) + abs5b
    END IF
    abs4b = k_sensor1b/abs22
    abs22b = -(abs4*k_sensor1b/abs22**2)
...
&       diffro+eps4*predro2)*dissro2b
      ...
      END IF
      coefb = k2*eps2b
      max2b = max11*max20*coefb
      max11b = max2*max20*coefb
      max20b = max2*max11*coefb
...
&       1.d-15))
      ...
      gradvb(i, j, 2) = gradvb(i, j, 2) + divub
      divu = gradu(i-1, j, 1) + gradv(i-1, j, 2)
      abs3b = k_sensor2b/abs21
      abs21b = -(abs3*k_sensor2b/abs21**2)
      ...
      ELSE
        ...
        pb(i, j) = pb(i, j) + abs3b
      END IF
      abs2b = k_sensor1b/abs20
      abs20b = -(abs2*k_sensor1b/abs20**2)
...
&       diffro+eps4*predro1)*dissro1b
      ...
      coefb = k2*eps2b
      max1b = max10*max19*coefb
      max10b = max1*max19*coefb
      max19b = max1*max10*coefb
...
&       1.d-15))
      ...
      gradub(i, j, 1) = gradub(i, j, 1) + divub
      gradvb(i, j, 2) = gradvb(i, j, 2) + divub
      abs1b = k_sensor2b/abs19
      abs19b = -(abs1*k_sensor2b/abs19**2)
      ...
      ELSE
        ...
        pb(i, j) = pb(i, j) + abs1b
      END IF
      abs0b = k_sensor1b/abs18
      abs18b = -(abs0*k_sensor1b/abs18**2)
...
&       (i, j)
    ...
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC7_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
