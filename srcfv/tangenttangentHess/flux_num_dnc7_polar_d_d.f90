...
!
!  Differentiation of flux_num_dnc7_polar_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs
! with!StaticTaping):
...
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of flux_num_dnc7_polar_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs w
!ith!StaticTaping):
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC7_POLAR_2D_D_D(residu, residud, residudd, w, wd0&
...
&       velxdd(i-2, j)) + b3*(velxdd(i+3, j)-velxdd(i-3, j))
...
&       velxd0(i-2, j)) + b3*(velxd0(i+3, j)-velxd0(i-3, j))
...
&       velydd(i-2, j)) + b3*(velydd(i+3, j)-velydd(i-3, j))
...
&       velyd0(i-2, j)) + b3*(velyd0(i+3, j)-velyd0(i-3, j))
...
&       velxd0(i, j-2)) + b3*(velxd0(i, j+3)-velxd0(i, j-3))
...
&       velydd(i, j-2)) + b3*(velydd(i, j+3)-velydd(i, j-3))
...
&       velyd0(i, j-2)) + b3*(velyd0(i, j+3)-velyd0(i, j-3))
...
!
      ...
      ELSE
        ...
        abs0 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
      END IF
      x10dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
      x10d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
      x10d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
      temp3 = abs0*abs18d/abs18
      temp2 = (abs0d-temp3)/abs18
      k_sensor1dd = (abs0dd-(abs18d*abs0d0+abs0*abs18dd-temp3*abs18d0)/&
&       abs18-temp2*abs18d0)/abs18
      k_sensor1d = temp2
      k_sensor1d0 = (abs0d0-abs0*abs18d0/abs18)/abs18
      ...
      ELSE
        ...
        abs1 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
      END IF
      x11dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
      x11d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
      x11d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
      ...
      ELSE
        ...
        abs19 = -x11
      END IF
      temp3 = abs1*abs19d/abs19
      temp2 = (abs1d-temp3)/abs19
      k_sensor2dd = (abs1dd-(abs19d*abs1d0+abs1*abs19dd-temp3*abs19d0)/&
&       abs19-temp2*abs19d0)/abs19
      k_sensor2d = temp2
      k_sensor2d0 = (abs1d0-abs1*abs19d0/abs19)/abs19
...
!                 k_sensor2*ducros2*dxm2)
      temp3 = max10*max1d + max1*max10d
      coefdd = temp3*max19d0 + max19*(max1d*max10d0+max10*max1dd+max10d*&
&       max1d0+max1*max10dd) + max19d*(max10*max1d0+max1*max10d0) + max1&
&       *max10*max19dd
      coefd = max19*temp3 + max1*max10*max19d
      coefd0 = max19*(max10*max1d0+max1*max10d0) + max1*max10*max19d0
...
!
      ...
      ELSE
        ...
        abs2 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
      END IF
      x12dd = pdd(i, j-1) + two*pdd(i, j) + pdd(i, j+1)
      x12d = pd(i, j-1) + two*pd(i, j) + pd(i, j+1)
      x12d0 = pd0(i, j-1) + two*pd0(i, j) + pd0(i, j+1)
...
!
! 2nd direction
      temp3 = abs2*abs20d/abs20
      temp2 = (abs2d-temp3)/abs20
      k_sensor1dd = (abs2dd-(abs20d*abs2d0+abs2*abs20dd-temp3*abs20d0)/&
&       abs20-temp2*abs20d0)/abs20
      k_sensor1d = temp2
      k_sensor1d0 = (abs2d0-abs2*abs20d0/abs20)/abs20
      ...
      ELSE
        ...
        abs3 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
      END IF
      x13dd = pdd(i, j-2) + two*pdd(i, j-1) + pdd(i, j)
      x13d = pd(i, j-2) + two*pd(i, j-1) + pd(i, j)
      x13d0 = pd0(i, j-2) + two*pd0(i, j-1) + pd0(i, j)
      ...
      ELSE
        ...
        abs21 = -x13
      END IF
      temp3 = abs3*abs21d/abs21
      temp2 = (abs3d-temp3)/abs21
      k_sensor2dd = (abs3dd-(abs21d*abs3d0+abs3*abs21dd-temp3*abs21d0)/&
&       abs21-temp2*abs21d0)/abs21
      k_sensor2d = temp2
      k_sensor2d0 = (abs3d0-abs3*abs21d0/abs21)/abs21
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
      temp3 = max11*max2d + max2*max11d
      coefdd = temp3*max20d0 + max20*(max2d*max11d0+max11*max2dd+max11d*&
&       max2d0+max2*max11dd) + max20d*(max11*max2d0+max2*max11d0) + max2&
&       *max11*max20dd
      coefd = max20*temp3 + max2*max11*max20d
      coefd0 = max20*(max11*max2d0+max2*max11d0) + max2*max11*max20d0
...
&     (i+3, j, 1)+fdd(i-4, j, 1))) + ny(i, j, 1)*(c1*(gdd(i, j, 1)+gdd(i&
&     -1, j, 1))+c2*(gdd(i+1, j, 1)+gdd(i-2, j, 1))+c3*(gdd(i+2, j, 1)+&
&     gdd(i-3, j, 1))+c4*(gdd(i+3, j, 1)+gdd(i-4, j, 1)))
    fxrou1dd = nx(i, j, 1)*(c1*(fdd(i, j, 2)+fdd(i-1, j, 2))+c2*(fdd(i+1&
&     , j, 2)+fdd(i-2, j, 2))+c3*(fdd(i+2, j, 2)+fdd(i-3, j, 2))+c4*(fdd&
&     (i+3, j, 2)+fdd(i-4, j, 2))) + ny(i, j, 1)*(c1*(gdd(i, j, 2)+gdd(i&
&     -1, j, 2))+c2*(gdd(i+1, j, 2)+gdd(i-2, j, 2))+c3*(gdd(i+2, j, 2)+&
&     gdd(i-3, j, 2))+c4*(gdd(i+3, j, 2)+gdd(i-4, j, 2)))
    fxrov1dd = nx(i, j, 1)*(c1*(fdd(i, j, 3)+fdd(i-1, j, 3))+c2*(fdd(i+1&
&     , j, 3)+fdd(i-2, j, 3))+c3*(fdd(i+2, j, 3)+fdd(i-3, j, 3))+c4*(fdd&
&     (i+3, j, 3)+fdd(i-4, j, 3))) + ny(i, j, 1)*(c1*(gdd(i, j, 3)+gdd(i&
&     -1, j, 3))+c2*(gdd(i+1, j, 3)+gdd(i-2, j, 3))+c3*(gdd(i+2, j, 3)+&
&     gdd(i-3, j, 3))+c4*(gdd(i+3, j, 3)+gdd(i-4, j, 3)))
    fxrow1dd = nx(i, j, 1)*(c1*(fdd(i, j, 4)+fdd(i-1, j, 4))+c2*(fdd(i+1&
&     , j, 4)+fdd(i-2, j, 4))+c3*(fdd(i+2, j, 4)+fdd(i-3, j, 4))+c4*(fdd&
&     (i+3, j, 4)+fdd(i-4, j, 4))) + ny(i, j, 1)*(c1*(gdd(i, j, 4)+gdd(i&
&     -1, j, 4))+c2*(gdd(i+1, j, 4)+gdd(i-2, j, 4))+c3*(gdd(i+2, j, 4)+&
&     gdd(i-3, j, 4))+c4*(gdd(i+3, j, 4)+gdd(i-4, j, 4)))
    fxroe1dd = nx(i, j, 1)*(c1*(fdd(i, j, 5)+fdd(i-1, j, 5))+c2*(fdd(i+1&
&     , j, 5)+fdd(i-2, j, 5))+c3*(fdd(i+2, j, 5)+fdd(i-3, j, 5))+c4*(fdd&
&     (i+3, j, 5)+fdd(i-4, j, 5))) + ny(i, j, 1)*(c1*(gdd(i, j, 5)+gdd(i&
&     -1, j, 5))+c2*(gdd(i+1, j, 5)+gdd(i-2, j, 5))+c3*(gdd(i+2, j, 5)+&
&     gdd(i-3, j, 5))+c4*(gdd(i+3, j, 5)+gdd(i-4, j, 5)))
    fxro2dd = nx(i, j, 2)*(c7d0*fdd(i, 1, 1)+c7d1*fdd(i, 2, 1)+c7d2*fdd(&
&     i, 3, 1)+c7d3*fdd(i, 4, 1)+c7d4*fdd(i, 5, 1)+c7d5*fdd(i, 6, 1)+&
&     c7d6*fdd(i, 7, 1)+c7d7*fdd(i, 8, 1)+c7d8*fdd(i, 9, 1)) + ny(i, j, &
&     2)*(c7d0*gdd(i, 1, 1)+c7d1*gdd(i, 2, 1)+c7d2*gdd(i, 3, 1)+c7d3*gdd&
&     (i, 4, 1)+c7d4*gdd(i, 5, 1)+c7d5*gdd(i, 6, 1)+c7d6*gdd(i, 7, 1)+&
&     c7d7*gdd(i, 8, 1)+c7d8*gdd(i, 9, 1))
    fxrou2dd = nx(i, j, 2)*(c7d0*fdd(i, 1, 2)+c7d1*fdd(i, 2, 2)+c7d2*fdd&
&     (i, 3, 2)+c7d3*fdd(i, 4, 2)+c7d4*fdd(i, 5, 2)+c7d5*fdd(i, 6, 2)+&
&     c7d6*fdd(i, 7, 2)+c7d7*fdd(i, 8, 2)+c7d8*fdd(i, 9, 2)) + ny(i, j, &
&     2)*(c7d0*gdd(i, 1, 2)+c7d1*gdd(i, 2, 2)+c7d2*gdd(i, 3, 2)+c7d3*gdd&
&     (i, 4, 2)+c7d4*gdd(i, 5, 2)+c7d5*gdd(i, 6, 2)+c7d6*gdd(i, 7, 2)+&
&     c7d7*gdd(i, 8, 2)+c7d8*gdd(i, 9, 2))
    fxrov2dd = nx(i, j, 2)*(c7d0*fdd(i, 1, 3)+c7d1*fdd(i, 2, 3)+c7d2*fdd&
&     (i, 3, 3)+c7d3*fdd(i, 4, 3)+c7d4*fdd(i, 5, 3)+c7d5*fdd(i, 6, 3)+&
&     c7d6*fdd(i, 7, 3)+c7d7*fdd(i, 8, 3)+c7d8*fdd(i, 9, 3)) + ny(i, j, &
&     2)*(c7d0*gdd(i, 1, 3)+c7d1*gdd(i, 2, 3)+c7d2*gdd(i, 3, 3)+c7d3*gdd&
&     (i, 4, 3)+c7d4*gdd(i, 5, 3)+c7d5*gdd(i, 6, 3)+c7d6*gdd(i, 7, 3)+&
&     c7d7*gdd(i, 8, 3)+c7d8*gdd(i, 9, 3))
    fxrow2dd = nx(i, j, 2)*(c7d0*fdd(i, 1, 4)+c7d1*fdd(i, 2, 4)+c7d2*fdd&
&     (i, 3, 4)+c7d3*fdd(i, 4, 4)+c7d4*fdd(i, 5, 4)+c7d5*fdd(i, 6, 4)+&
&     c7d6*fdd(i, 7, 4)+c7d7*fdd(i, 8, 4)+c7d8*fdd(i, 9, 4)) + ny(i, j, &
&     2)*(c7d0*gdd(i, 1, 4)+c7d1*gdd(i, 2, 4)+c7d2*gdd(i, 3, 4)+c7d3*gdd&
&     (i, 4, 4)+c7d4*gdd(i, 5, 4)+c7d5*gdd(i, 6, 4)+c7d6*gdd(i, 7, 4)+&
&     c7d7*gdd(i, 8, 4)+c7d8*gdd(i, 9, 4))
    fxroe2dd = nx(i, j, 2)*(c7d0*fdd(i, 1, 5)+c7d1*fdd(i, 2, 5)+c7d2*fdd&
&     (i, 3, 5)+c7d3*fdd(i, 4, 5)+c7d4*fdd(i, 5, 5)+c7d5*fdd(i, 6, 5)+&
&     c7d6*fdd(i, 7, 5)+c7d7*fdd(i, 8, 5)+c7d8*fdd(i, 9, 5)) + ny(i, j, &
&     2)*(c7d0*gdd(i, 1, 5)+c7d1*gdd(i, 2, 5)+c7d2*gdd(i, 3, 5)+c7d3*gdd&
&     (i, 4, 5)+c7d4*gdd(i, 5, 5)+c7d5*gdd(i, 6, 5)+c7d6*gdd(i, 7, 5)+&
&     c7d7*gdd(i, 8, 5)+c7d8*gdd(i, 9, 5))
    predro1d = d4*wd(i-4, j, 1) - d3*wd(i-3, j, 1) + d2*wd(i-2, j, 1) - &
&     d1*wd(i-1, j, 1) + d1*wd(i, j, 1) - d2*wd(i+1, j, 1) + d3*wd(i+2, &
&     j, 1) - d4*wd(i+3, j, 1)
    predro1d0 = d4*wd0(i-4, j, 1) - d3*wd0(i-3, j, 1) + d2*wd0(i-2, j, 1&
&     ) - d1*wd0(i-1, j, 1) + d1*wd0(i, j, 1) - d2*wd0(i+1, j, 1) + d3*&
&     wd0(i+2, j, 1) - d4*wd0(i+3, j, 1)
...
&     j, 2) - d4*wd(i+3, j, 2)
    predrou1d0 = d4*wd0(i-4, j, 2) - d3*wd0(i-3, j, 2) + d2*wd0(i-2, j, &
&     2) - d1*wd0(i-1, j, 2) + d1*wd0(i, j, 2) - d2*wd0(i+1, j, 2) + d3*&
&     wd0(i+2, j, 2) - d4*wd0(i+3, j, 2)
...
&     j, 3) - d4*wd(i+3, j, 3)
    predrov1d0 = d4*wd0(i-4, j, 3) - d3*wd0(i-3, j, 3) + d2*wd0(i-2, j, &
&     3) - d1*wd0(i-1, j, 3) + d1*wd0(i, j, 3) - d2*wd0(i+1, j, 3) + d3*&
&     wd0(i+2, j, 3) - d4*wd0(i+3, j, 3)
...
&     j, 4) - d4*wd(i+3, j, 4)
    predrow1d0 = d4*wd0(i-4, j, 4) - d3*wd0(i-3, j, 4) + d2*wd0(i-2, j, &
&     4) - d1*wd0(i-1, j, 4) + d1*wd0(i, j, 4) - d2*wd0(i+1, j, 4) + d3*&
&     wd0(i+2, j, 4) - d4*wd0(i+3, j, 4)
...
&     j, 5) - d4*wd(i+3, j, 5)
    predroe1d0 = d4*wd0(i-4, j, 5) - d3*wd0(i-3, j, 5) + d2*wd0(i-2, j, &
&     5) - d1*wd0(i-1, j, 5) + d1*wd0(i, j, 5) - d2*wd0(i+1, j, 5) + d3*&
&     wd0(i+2, j, 5) - d4*wd0(i+3, j, 5)
...
&     2, 1) - d4*wd(i, j+3, 1)
    predro2d0 = d4*wd0(i, j-4, 1) - d3*wd0(i, j-3, 1) + d2*wd0(i, j-2, 1&
&     ) - d1*wd0(i, j-1, 1) + d1*wd0(i, j, 1) - d2*wd0(i, j+1, 1) + d3*&
&     wd0(i, j+2, 1) - d4*wd0(i, j+3, 1)
...
&     2, 2) - d4*wd(i, j+3, 2)
    predrou2d0 = d4*wd0(i, j-4, 2) - d3*wd0(i, j-3, 2) + d2*wd0(i, j-2, &
&     2) - d1*wd0(i, j-1, 2) + d1*wd0(i, j, 2) - d2*wd0(i, j+1, 2) + d3*&
&     wd0(i, j+2, 2) - d4*wd0(i, j+3, 2)
...
&     2, 3) - d4*wd(i, j+3, 3)
    predrov2d0 = d4*wd0(i, j-4, 3) - d3*wd0(i, j-3, 3) + d2*wd0(i, j-2, &
&     3) - d1*wd0(i, j-1, 3) + d1*wd0(i, j, 3) - d2*wd0(i, j+1, 3) + d3*&
&     wd0(i, j+2, 3) - d4*wd0(i, j+3, 3)
...
&     2, 4) - d4*wd(i, j+3, 4)
    predrow2d0 = d4*wd0(i, j-4, 4) - d3*wd0(i, j-3, 4) + d2*wd0(i, j-2, &
&     4) - d1*wd0(i, j-1, 4) + d1*wd0(i, j, 4) - d2*wd0(i, j+1, 4) + d3*&
&     wd0(i, j+2, 4) - d4*wd0(i, j+3, 4)
...
&     2, 5) - d4*wd(i, j+3, 5)
    predroe2d0 = d4*wd0(i, j-4, 5) - d3*wd0(i, j-3, 5) + d2*wd0(i, j-2, &
&     5) - d1*wd0(i, j-1, 5) + d1*wd0(i, j, 5) - d2*wd0(i, j+1, 5) + d3*&
&     wd0(i, j+2, 5) - d4*wd0(i, j+3, 5)
...
!
    ...
    ELSE
      ...
      abs4 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
    x14dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
    x14d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
    x14d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
    temp3 = abs4*abs22d/abs22
    temp2 = (abs4d-temp3)/abs22
    k_sensor1dd = (abs4dd-(abs22d*abs4d0+abs4*abs22dd-temp3*abs22d0)/&
&     abs22-temp2*abs22d0)/abs22
    k_sensor1d = temp2
    k_sensor1d0 = (abs4d0-abs4*abs22d0/abs22)/abs22
    ...
    ELSE
      ...
      abs5 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    x15dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
    x15d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
    x15d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
    ...
    ELSE
      ...
      abs23 = -x15
    END IF
    temp3 = abs5*abs23d/abs23
    temp2 = (abs5d-temp3)/abs23
    k_sensor2dd = (abs5dd-(abs23d*abs5d0+abs5*abs23dd-temp3*abs23d0)/&
&     abs23-temp2*abs23d0)/abs23
    k_sensor2d = temp2
    k_sensor2d0 = (abs5d0-abs5*abs23d0/abs23)/abs23
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max12*max3d + max3*max12d
    coefdd = temp3*max21d0 + max21*(max3d*max12d0+max12*max3dd+max12d*&
&     max3d0+max3*max12dd) + max21d*(max12*max3d0+max3*max12d0) + max3*&
&     max12*max21dd
    coefd = max21*temp3 + max3*max12*max21d
    coefd0 = max21*(max12*max3d0+max3*max12d0) + max3*max12*max21d0
...
!
    ...
    ELSE
      ...
      abs6 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
    x16dd = pdd(i, j-1) + two*pdd(i, j) + pdd(i, j+1)
    x16d = pd(i, j-1) + two*pd(i, j) + pd(i, j+1)
    x16d0 = pd0(i, j-1) + two*pd0(i, j) + pd0(i, j+1)
...
!
! 2nd direction
    temp3 = abs6*abs24d/abs24
    temp2 = (abs6d-temp3)/abs24
    k_sensor1dd = (abs6dd-(abs24d*abs6d0+abs6*abs24dd-temp3*abs24d0)/&
&     abs24-temp2*abs24d0)/abs24
    k_sensor1d = temp2
    k_sensor1d0 = (abs6d0-abs6*abs24d0/abs24)/abs24
    ...
    ELSE
      ...
      abs7 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    x17dd = pdd(i, j-2) + two*pdd(i, j-1) + pdd(i, j)
    x17d = pd(i, j-2) + two*pd(i, j-1) + pd(i, j)
    x17d0 = pd0(i, j-2) + two*pd0(i, j-1) + pd0(i, j)
    ...
    ELSE
      ...
      abs25 = -x17
    END IF
    temp3 = abs7*abs25d/abs25
    temp2 = (abs7d-temp3)/abs25
    k_sensor2dd = (abs7dd-(abs25d*abs7d0+abs7*abs25dd-temp3*abs25d0)/&
&     abs25-temp2*abs25d0)/abs25
    k_sensor2d = temp2
    k_sensor2d0 = (abs7d0-abs7*abs25d0/abs25)/abs25
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max13*max4d + max4*max13d
    coefdd = temp3*max22d0 + max22*(max4d*max13d0+max13*max4dd+max13d*&
&     max4d0+max4*max13dd) + max22d*(max13*max4d0+max4*max13d0) + max4*&
&     max13*max22dd
    coefd = max22*temp3 + max4*max13*max22d
    coefd0 = max22*(max13*max4d0+max4*max13d0) + max4*max13*max22d0
...
&     c5d6*fdd(i, 7, 1)+c5d7*fdd(i, 8, 1)+c5d8*fdd(i, 9, 1)) + ny(i, j, &
&     2)*(c5d0*gdd(i, 1, 1)+c5d1*gdd(i, 2, 1)+c5d2*gdd(i, 3, 1)+c5d3*gdd&
&     (i, 4, 1)+c5d4*gdd(i, 5, 1)+c5d5*gdd(i, 6, 1)+c5d6*gdd(i, 7, 1)+&
&     c5d7*gdd(i, 8, 1)+c5d8*gdd(i, 9, 1))
    fxrou2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 2)+c5d1*fdd(i, 2, 2)+c5d2*fdd&
&     (i, 3, 2)+c5d3*fdd(i, 4, 2)+c5d4*fdd(i, 5, 2)+c5d5*fdd(i, 6, 2)+&
&     c5d6*fdd(i, 7, 2)+c5d7*fdd(i, 8, 2)+c5d8*fdd(i, 9, 2)) + ny(i, j, &
&     2)*(c5d0*gdd(i, 1, 2)+c5d1*gdd(i, 2, 2)+c5d2*gdd(i, 3, 2)+c5d3*gdd&
&     (i, 4, 2)+c5d4*gdd(i, 5, 2)+c5d5*gdd(i, 6, 2)+c5d6*gdd(i, 7, 2)+&
&     c5d7*gdd(i, 8, 2)+c5d8*gdd(i, 9, 2))
    fxrov2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 3)+c5d1*fdd(i, 2, 3)+c5d2*fdd&
&     (i, 3, 3)+c5d3*fdd(i, 4, 3)+c5d4*fdd(i, 5, 3)+c5d5*fdd(i, 6, 3)+&
&     c5d6*fdd(i, 7, 3)+c5d7*fdd(i, 8, 3)+c5d8*fdd(i, 9, 3)) + ny(i, j, &
&     2)*(c5d0*gdd(i, 1, 3)+c5d1*gdd(i, 2, 3)+c5d2*gdd(i, 3, 3)+c5d3*gdd&
&     (i, 4, 3)+c5d4*gdd(i, 5, 3)+c5d5*gdd(i, 6, 3)+c5d6*gdd(i, 7, 3)+&
&     c5d7*gdd(i, 8, 3)+c5d8*gdd(i, 9, 3))
    fxrow2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 4)+c5d1*fdd(i, 2, 4)+c5d2*fdd&
&     (i, 3, 4)+c5d3*fdd(i, 4, 4)+c5d4*fdd(i, 5, 4)+c5d5*fdd(i, 6, 4)+&
&     c5d6*fdd(i, 7, 4)+c5d7*fdd(i, 8, 4)+c5d8*fdd(i, 9, 4)) + ny(i, j, &
&     2)*(c5d0*gdd(i, 1, 4)+c5d1*gdd(i, 2, 4)+c5d2*gdd(i, 3, 4)+c5d3*gdd&
&     (i, 4, 4)+c5d4*gdd(i, 5, 4)+c5d5*gdd(i, 6, 4)+c5d6*gdd(i, 7, 4)+&
&     c5d7*gdd(i, 8, 4)+c5d8*gdd(i, 9, 4))
    fxroe2dd = nx(i, j, 2)*(c5d0*fdd(i, 1, 5)+c5d1*fdd(i, 2, 5)+c5d2*fdd&
&     (i, 3, 5)+c5d3*fdd(i, 4, 5)+c5d4*fdd(i, 5, 5)+c5d5*fdd(i, 6, 5)+&
&     c5d6*fdd(i, 7, 5)+c5d7*fdd(i, 8, 5)+c5d8*fdd(i, 9, 5)) + ny(i, j, &
&     2)*(c5d0*gdd(i, 1, 5)+c5d1*gdd(i, 2, 5)+c5d2*gdd(i, 3, 5)+c5d3*gdd&
&     (i, 4, 5)+c5d4*gdd(i, 5, 5)+c5d5*gdd(i, 6, 5)+c5d6*gdd(i, 7, 5)+&
&     c5d7*gdd(i, 8, 5)+c5d8*gdd(i, 9, 5))
    predro1d = d4*wd(i-4, j, 1) - d3*wd(i-3, j, 1) + d2*wd(i-2, j, 1) - &
&     d1*wd(i-1, j, 1) + d1*wd(i, j, 1) - d2*wd(i+1, j, 1) + d3*wd(i+2, &
&     j, 1) - d4*wd(i+3, j, 1)
    predro1d0 = d4*wd0(i-4, j, 1) - d3*wd0(i-3, j, 1) + d2*wd0(i-2, j, 1&
...
!
    ...
    ELSE
      ...
      abs8 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
    x18dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
    x18d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
    x18d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
    temp3 = abs8*abs26d/abs26
    temp2 = (abs8d-temp3)/abs26
    k_sensor1dd = (abs8dd-(abs26d*abs8d0+abs8*abs26dd-temp3*abs26d0)/&
&     abs26-temp2*abs26d0)/abs26
    k_sensor1d = temp2
    k_sensor1d0 = (abs8d0-abs8*abs26d0/abs26)/abs26
    ...
    ELSE
      ...
      abs9 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    x19dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
    x19d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
    x19d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
    ...
    ELSE
      ...
      abs27 = -x19
    END IF
    temp3 = abs9*abs27d/abs27
    temp2 = (abs9d-temp3)/abs27
    k_sensor2dd = (abs9dd-(abs27d*abs9d0+abs9*abs27dd-temp3*abs27d0)/&
&     abs27-temp2*abs27d0)/abs27
    k_sensor2d = temp2
    k_sensor2d0 = (abs9d0-abs9*abs27d0/abs27)/abs27
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max14*max5d + max5*max14d
    coefdd = temp3*max23d0 + max23*(max5d*max14d0+max14*max5dd+max14d*&
&     max5d0+max5*max14dd) + max23d*(max14*max5d0+max5*max14d0) + max5*&
&     max14*max23dd
    coefd = max23*temp3 + max5*max14*max23d
    coefd0 = max23*(max14*max5d0+max5*max14d0) + max5*max14*max23d0
...
!
    ...
    ELSE
      ...
      abs10 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
    x20dd = pdd(i, j-1) + two*pdd(i, j) + pdd(i, j+1)
    x20d = pd(i, j-1) + two*pd(i, j) + pd(i, j+1)
    x20d0 = pd0(i, j-1) + two*pd0(i, j) + pd0(i, j+1)
...
!
! 2nd direction
    temp3 = abs10*abs28d/abs28
    temp2 = (abs10d-temp3)/abs28
    k_sensor1dd = (abs10dd-(abs28d*abs10d0+abs10*abs28dd-temp3*abs28d0)/&
&     abs28-temp2*abs28d0)/abs28
    k_sensor1d = temp2
    k_sensor1d0 = (abs10d0-abs10*abs28d0/abs28)/abs28
    ...
    ELSE
      ...
      abs11 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    x21dd = pdd(i, j-2) + two*pdd(i, j-1) + pdd(i, j)
    x21d = pd(i, j-2) + two*pd(i, j-1) + pd(i, j)
    x21d0 = pd0(i, j-2) + two*pd0(i, j-1) + pd0(i, j)
    ...
    ELSE
      ...
      abs29 = -x21
    END IF
    temp3 = abs11*abs29d/abs29
    temp2 = (abs11d-temp3)/abs29
    k_sensor2dd = (abs11dd-(abs29d*abs11d0+abs11*abs29dd-temp3*abs29d0)/&
&     abs29-temp2*abs29d0)/abs29
    k_sensor2d = temp2
    k_sensor2d0 = (abs11d0-abs11*abs29d0/abs29)/abs29
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max15*max6d + max6*max15d
    coefdd = temp3*max24d0 + max24*(max6d*max15d0+max15*max6dd+max15d*&
&     max6d0+max6*max15dd) + max24d*(max15*max6d0+max6*max15d0) + max6*&
&     max15*max24dd
    coefd = max24*temp3 + max6*max15*max24d
    coefd0 = max24*(max15*max6d0+max6*max15d0) + max6*max15*max24d0
...
&     c3d6*fdd(i, 7, 1)+c3d7*fdd(i, 8, 1)+c3d8*fdd(i, 9, 1)) + ny(i, j, &
&     2)*(c3d0*gdd(i, 1, 1)+c3d1*gdd(i, 2, 1)+c3d2*gdd(i, 3, 1)+c3d3*gdd&
&     (i, 4, 1)+c3d4*gdd(i, 5, 1)+c3d5*gdd(i, 6, 1)+c3d6*gdd(i, 7, 1)+&
&     c3d7*gdd(i, 8, 1)+c3d8*gdd(i, 9, 1))
    fxrou2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 2)+c3d1*fdd(i, 2, 2)+c3d2*fdd&
&     (i, 3, 2)+c3d3*fdd(i, 4, 2)+c3d4*fdd(i, 5, 2)+c3d5*fdd(i, 6, 2)+&
&     c3d6*fdd(i, 7, 2)+c3d7*fdd(i, 8, 2)+c3d8*fdd(i, 9, 2)) + ny(i, j, &
&     2)*(c3d0*gdd(i, 1, 2)+c3d1*gdd(i, 2, 2)+c3d2*gdd(i, 3, 2)+c3d3*gdd&
&     (i, 4, 2)+c3d4*gdd(i, 5, 2)+c3d5*gdd(i, 6, 2)+c3d6*gdd(i, 7, 2)+&
&     c3d7*gdd(i, 8, 2)+c3d8*gdd(i, 9, 2))
    fxrov2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 3)+c3d1*fdd(i, 2, 3)+c3d2*fdd&
&     (i, 3, 3)+c3d3*fdd(i, 4, 3)+c3d4*fdd(i, 5, 3)+c3d5*fdd(i, 6, 3)+&
&     c3d6*fdd(i, 7, 3)+c3d7*fdd(i, 8, 3)+c3d8*fdd(i, 9, 3)) + ny(i, j, &
&     2)*(c3d0*gdd(i, 1, 3)+c3d1*gdd(i, 2, 3)+c3d2*gdd(i, 3, 3)+c3d3*gdd&
&     (i, 4, 3)+c3d4*gdd(i, 5, 3)+c3d5*gdd(i, 6, 3)+c3d6*gdd(i, 7, 3)+&
&     c3d7*gdd(i, 8, 3)+c3d8*gdd(i, 9, 3))
    fxrow2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 4)+c3d1*fdd(i, 2, 4)+c3d2*fdd&
&     (i, 3, 4)+c3d3*fdd(i, 4, 4)+c3d4*fdd(i, 5, 4)+c3d5*fdd(i, 6, 4)+&
&     c3d6*fdd(i, 7, 4)+c3d7*fdd(i, 8, 4)+c3d8*fdd(i, 9, 4)) + ny(i, j, &
&     2)*(c3d0*gdd(i, 1, 4)+c3d1*gdd(i, 2, 4)+c3d2*gdd(i, 3, 4)+c3d3*gdd&
&     (i, 4, 4)+c3d4*gdd(i, 5, 4)+c3d5*gdd(i, 6, 4)+c3d6*gdd(i, 7, 4)+&
&     c3d7*gdd(i, 8, 4)+c3d8*gdd(i, 9, 4))
    fxroe2dd = nx(i, j, 2)*(c3d0*fdd(i, 1, 5)+c3d1*fdd(i, 2, 5)+c3d2*fdd&
&     (i, 3, 5)+c3d3*fdd(i, 4, 5)+c3d4*fdd(i, 5, 5)+c3d5*fdd(i, 6, 5)+&
&     c3d6*fdd(i, 7, 5)+c3d7*fdd(i, 8, 5)+c3d8*fdd(i, 9, 5)) + ny(i, j, &
&     2)*(c3d0*gdd(i, 1, 5)+c3d1*gdd(i, 2, 5)+c3d2*gdd(i, 3, 5)+c3d3*gdd&
&     (i, 4, 5)+c3d4*gdd(i, 5, 5)+c3d5*gdd(i, 6, 5)+c3d6*gdd(i, 7, 5)+&
&     c3d7*gdd(i, 8, 5)+c3d8*gdd(i, 9, 5))
    predro1d = d4*wd(i-4, j, 1) - d3*wd(i-3, j, 1) + d2*wd(i-2, j, 1) - &
&     d1*wd(i-1, j, 1) + d1*wd(i, j, 1) - d2*wd(i+1, j, 1) + d3*wd(i+2, &
&     j, 1) - d4*wd(i+3, j, 1)
    predro1d0 = d4*wd0(i-4, j, 1) - d3*wd0(i-3, j, 1) + d2*wd0(i-2, j, 1&
...
!
    ...
    ELSE
      ...
      abs12 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
    x22dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
    x22d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
    x22d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
    temp3 = abs12*abs30d/abs30
    temp2 = (abs12d-temp3)/abs30
    k_sensor1dd = (abs12dd-(abs30d*abs12d0+abs12*abs30dd-temp3*abs30d0)/&
&     abs30-temp2*abs30d0)/abs30
    k_sensor1d = temp2
    k_sensor1d0 = (abs12d0-abs12*abs30d0/abs30)/abs30
    ...
    ELSE
      ...
      abs13 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    x23dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
    x23d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
    x23d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
    ...
    ELSE
      ...
      abs31 = -x23
    END IF
    temp3 = abs13*abs31d/abs31
    temp2 = (abs13d-temp3)/abs31
    k_sensor2dd = (abs13dd-(abs31d*abs13d0+abs13*abs31dd-temp3*abs31d0)/&
&     abs31-temp2*abs31d0)/abs31
    k_sensor2d = temp2
    k_sensor2d0 = (abs13d0-abs13*abs31d0/abs31)/abs31
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max16*max7d + max7*max16d
    coefdd = temp3*max25d0 + max25*(max7d*max16d0+max16*max7dd+max16d*&
&     max7d0+max7*max16dd) + max25d*(max16*max7d0+max7*max16d0) + max7*&
&     max16*max25dd
    coefd = max25*temp3 + max7*max16*max25d
    coefd0 = max25*(max16*max7d0+max7*max16d0) + max7*max16*max25d0
...
!
    ...
    ELSE
      ...
      abs14 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
    x24dd = pdd(i, j-1) + two*pdd(i, j) + pdd(i, j+1)
    x24d = pd(i, j-1) + two*pd(i, j) + pd(i, j+1)
    x24d0 = pd0(i, j-1) + two*pd0(i, j) + pd0(i, j+1)
...
!
! 2nd direction
    temp3 = abs14*abs32d/abs32
    temp2 = (abs14d-temp3)/abs32
    k_sensor1dd = (abs14dd-(abs32d*abs14d0+abs14*abs32dd-temp3*abs32d0)/&
&     abs32-temp2*abs32d0)/abs32
    k_sensor1d = temp2
    k_sensor1d0 = (abs14d0-abs14*abs32d0/abs32)/abs32
    ...
    ELSE
      ...
      abs15 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    x25dd = pdd(i, j-2) + two*pdd(i, j-1) + pdd(i, j)
    x25d = pd(i, j-2) + two*pd(i, j-1) + pd(i, j)
    x25d0 = pd0(i, j-2) + two*pd0(i, j-1) + pd0(i, j)
    ...
    ELSE
      ...
      abs33 = -x25
    END IF
    temp3 = abs15*abs33d/abs33
    temp2 = (abs15d-temp3)/abs33
    k_sensor2dd = (abs15dd-(abs33d*abs15d0+abs15*abs33dd-temp3*abs33d0)/&
&     abs33-temp2*abs33d0)/abs33
    k_sensor2d = temp2
    k_sensor2d0 = (abs15d0-abs15*abs33d0/abs33)/abs33
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max17*max8d + max8*max17d
    coefdd = temp3*max26d0 + max26*(max8d*max17d0+max17*max8dd+max17d*&
&     max8d0+max8*max17dd) + max26d*(max17*max8d0+max8*max17d0) + max8*&
&     max17*max26dd
    coefd = max26*temp3 + max8*max17*max26d
    coefd0 = max26*(max17*max8d0+max8*max17d0) + max8*max17*max26d0
...
!
    ...
    ELSE
      ...
      abs16 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
    x26dd = pdd(i-1, j) + two*pdd(i, j) + pdd(i+1, j)
    x26d = pd(i-1, j) + two*pd(i, j) + pd(i+1, j)
    x26d0 = pd0(i-1, j) + two*pd0(i, j) + pd0(i+1, j)
...
!
!1st direction
    temp3 = abs16*abs34d/abs34
    temp2 = (abs16d-temp3)/abs34
    k_sensor1dd = (abs16dd-(abs34d*abs16d0+abs16*abs34dd-temp3*abs34d0)/&
&     abs34-temp2*abs34d0)/abs34
    k_sensor1d = temp2
    k_sensor1d0 = (abs16d0-abs16*abs34d0/abs34)/abs34
    ...
    ELSE
      ...
      abs17 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    x27dd = pdd(i-2, j) + two*pdd(i-1, j) + pdd(i, j)
    x27d = pd(i-2, j) + two*pd(i-1, j) + pd(i, j)
    x27d0 = pd0(i-2, j) + two*pd0(i-1, j) + pd0(i, j)
    ...
    ELSE
      ...
      abs35 = -x27
    END IF
    temp3 = abs17*abs35d/abs35
    temp2 = (abs17d-temp3)/abs35
    k_sensor2dd = (abs17dd-(abs35d*abs17d0+abs17*abs35dd-temp3*abs35d0)/&
&     abs35-temp2*abs35d0)/abs35
    k_sensor2d = temp2
    k_sensor2d0 = (abs17d0-abs17*abs35d0/abs35)/abs35
...
!coef      = MAX( k_sensor1*ducros1*dxm1, &
!                 k_sensor2*ducros2*dxm2)
    temp3 = max18*max9d + max9*max18d
    coefdd = temp3*max27d0 + max27*(max9d*max18d0+max18*max9dd+max18d*&
&     max9d0+max9*max18dd) + max27d*(max18*max9d0+max9*max18d0) + max9*&
&     max18*max27dd
    coefd = max27*temp3 + max9*max18*max27d
    coefd0 = max27*(max18*max9d0+max9*max18d0) + max9*max18*max27d0
...
&       + hndd(i, j, 5, 2) - hndd(i, j+1, 5, 2)
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC7_POLAR_2D_D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
