...
!1st direction
      temp3 = abs2*abs22d/abs22
      temp2 = (abs2d-temp3)/abs22
      k_sensor1dd = (abs2dd-(abs22d*abs2d0+abs2*abs22dd-temp3*abs22d0)/&
&       abs22-temp2*abs22d0)/abs22
      k_sensor1d = temp2
      k_sensor1d0 = (abs2d0-abs2*abs22d0/abs22)/abs22
      k_sensor1 = abs2/abs22
      ...
      ELSE
        ...
        abs23 = -x9
      END IF
      temp3 = abs3*abs23d/abs23
      temp2 = (abs3d-temp3)/abs23
      k_sensor2dd = (abs3dd-(abs23d*abs3d0+abs3*abs23dd-temp3*abs23d0)/&
&       abs23-temp2*abs23d0)/abs23
      k_sensor2d = temp2
      k_sensor2d0 = (abs3d0-abs3*abs23d0/abs23)/abs23
      k_sensor2 = abs3/abs23
...
! 2nd direction
      temp3 = abs4*abs24d/abs24
      temp2 = (abs4d-temp3)/abs24
      k_sensor1dd = (abs4dd-(abs24d*abs4d0+abs4*abs24dd-temp3*abs24d0)/&
&       abs24-temp2*abs24d0)/abs24
      k_sensor1d = temp2
      k_sensor1d0 = (abs4d0-abs4*abs24d0/abs24)/abs24
      k_sensor1 = abs4/abs24
      ...
      ELSE
        ...
        abs25 = -x11
      END IF
      temp3 = abs5*abs25d/abs25
      temp2 = (abs5d-temp3)/abs25
      k_sensor2dd = (abs5dd-(abs25d*abs5d0+abs5*abs25dd-temp3*abs25d0)/&
&       abs25-temp2*abs25d0)/abs25
      k_sensor2d = temp2
      k_sensor2d0 = (abs5d0-abs5*abs25d0/abs25)/abs25
      k_sensor2 = abs5/abs25
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs14*abs30d/abs30
    temp2 = (abs14d-temp3)/abs30
    k_sensor1dd = (abs14dd-(abs30d*abs14d0+abs14*abs30dd-temp3*abs30d0)/&
&     abs30-temp2*abs30d0)/abs30
    k_sensor1d = temp2
    k_sensor1d0 = (abs14d0-abs14*abs30d0/abs30)/abs30
    k_sensor1 = abs14/abs30
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs31 = -x17
    END IF
    temp3 = abs15*abs31d/abs31
    temp2 = (abs15d-temp3)/abs31
    k_sensor2dd = (abs15dd-(abs31d*abs15d0+abs15*abs31dd-temp3*abs31d0)/&
&     abs31-temp2*abs31d0)/abs31
    k_sensor2d = temp2
    k_sensor2d0 = (abs15d0-abs15*abs31d0/abs31)/abs31
    k_sensor2 = abs15/abs31
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs16*abs32d/abs32
    temp2 = (abs16d-temp3)/abs32
    k_sensor1dd = (abs16dd-(abs32d*abs16d0+abs16*abs32dd-temp3*abs32d0)/&
&     abs32-temp2*abs32d0)/abs32
    k_sensor1d = temp2
    k_sensor1d0 = (abs16d0-abs16*abs32d0/abs32)/abs32
    k_sensor1 = abs16/abs32
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs33 = -x19
    END IF
    temp3 = abs17*abs33d/abs33
    temp2 = (abs17d-temp3)/abs33
    k_sensor2dd = (abs17dd-(abs33d*abs17d0+abs17*abs33dd-temp3*abs33d0)/&
&     abs33-temp2*abs33d0)/abs33
    k_sensor2d = temp2
    k_sensor2d0 = (abs17d0-abs17*abs33d0/abs33)/abs33
    k_sensor2 = abs17/abs33
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs19dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs19d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs19d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs19 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs19dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs19d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs19d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs19 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs19*abs34d/abs34
    temp2 = (abs19d-temp3)/abs34
    k_sensor1dd = (abs19dd-(abs34d*abs19d0+abs19*abs34dd-temp3*abs34d0)/&
&     abs34-temp2*abs34d0)/abs34
    k_sensor1d = temp2
    k_sensor1d0 = (abs19d0-abs19*abs34d0/abs34)/abs34
    k_sensor1 = abs19/abs34
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs20dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs20d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs20d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs20 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs20dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs20d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs20d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs20 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs35 = -x21
    END IF
    temp3 = abs20*abs35d/abs35
    temp2 = (abs20d-temp3)/abs35
    k_sensor2dd = (abs20dd-(abs35d*abs20d0+abs20*abs35dd-temp3*abs35d0)/&
&     abs35-temp2*abs35d0)/abs35
    k_sensor2d = temp2
    k_sensor2d0 = (abs20d0-abs20*abs35d0/abs35)/abs35
    k_sensor2 = abs20/abs35
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
