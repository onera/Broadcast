...
!
!  Differentiation of bc_wall_viscous_adia_cyl_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadIns
!trs with!StaticTaping):
!   gradient     of useful results: w
!   with respect to varying inputs: w
!   RW status of diff variables: w:in-out
SUBROUTINE BC_WALL_VISCOUS_ADIA_CYL_2D_B(w, wb, loc, gam, interf, nx, ny&
& , gh, im, jm)
  IMPLICIT NONE
!
!
...
! Local variables -------------------------------------------------
  ...
  REAL*8 :: ve2, gami, gam1, ct0, ct1, third, four, half, one, zero
  REAL*8 :: ve2b
  REAL*8 :: nxloc, nyloc, vii, vj, nsum, nsumi, nxnorm, nynorm, sens
  REAL*8 :: viib
! -----------------------------------------------------------------
! Local variables -----------------------------------------------------------
...
! vi = - ve
! wi = - we
    vii = SQRT(ue*ue*nxnorm*nxnorm + ve*ve*nynorm*nynorm)
    vj = zero
    ui = nxnorm*vii - nynorm*vj
    vi = nynorm*vii + nxnorm*vj
!
! roi = roe
! pi  = pe
! roiei  = pi/gam1 + HALF*roi*(ui*ui+vi*vi+wi*wi)
    roiei = pi/gam1 + half*roi*ve2
    DO de=1,gh
...
&       ui*wb(i-de*i0, j-de*j0, 2) + wb(i-de*i0, j-de*j0, 1)
    ...
    END DO
    roib = roib + ve2*half*roieib
    viib = nynorm*vib + nxnorm*uib
    IF (nxnorm**2*ue**2 + nynorm**2*ve**2 .EQ. 0.0) THEN
      tempb0 = 0.0_8
    ELSE
      tempb0 = viib/(2.0*SQRT(nxnorm**2*ue**2+nynorm**2*ve**2))
    END IF
    ueb = 2*ue*nxnorm**2*tempb0
    veb = 2*ve*nynorm**2*tempb0
    temp = pi/pe
    temp0 = roe**gam
...
&     j0, 2)*ue1b
    ...
    tempb = gam1*peb
    ve2b = roi*half*roieib - roe*half*tempb
...
&       roe**2
  ...
  END DO
END SUBROUTINE BC_WALL_VISCOUS_ADIA_CYL_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
