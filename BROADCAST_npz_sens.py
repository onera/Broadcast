...
import srcfv.f_chu as f_chu
import srcfv.f_linchu as f_linchu
...
def fillNPZ(filename, w, res, IA, JA, Jacvol, gh, wallprof=None):
...
def fillNPZ_dRdp(filename, IAdp, JAdp, Jacdp):
    ''' fill a given NPZ file with the lists (IA,JA,Aij) for the derivative of the residual with a flow parameter p'''

    dic = dict(_np.load(filename+'.npz', allow_pickle=True))
    dic['IAdRdp'] = IAdp
    dic['JAdRdp'] = JAdp
    dic['AijdRdp'] = Jacdp
    _np.savez(filename+'.npz', **dic)
...
def bl2d_fromNPZtree(treename, ite = 10, compmode = 'fixed_point', out_dir = 'totodir', isresol= False):
    ...
    if compmode == 'direct':
        ...
        ## Steady boundary profile to be presribed at the wall

        # twallprof = 4.395*_np.ones(im+2*gh)
        velprof   = _np.zeros((im+2*gh), order='F')
        # fluxprof  = _np.zeros((im+2*gh), order='F')
        ...
        for it in range(1,ite+1):
            ...
            foutflow(w,'Ihi', interf2, im, jm, gh)
            # fwall(w,'Jlo', gam, interf3, gh, im, jm)
            # fwall(w, twallprof,'Jlo', gam, rgaz, interf3, gh, im, jm)
            fwall(w, velprof,'Jlo', gam, interf3, gh, im, jm)
            # fwall(w, fluxprof,'Jlo', gam, rgaz, interf3, gh, im, jm)
                ...
                for m in range(5):
                    for l in range(1 + 2*gh):
                        for k in range(1 + 2*gh):
                            ...
                            # twallprofd = _np.zeros((im+2*gh), order='F')
                            # flinwall(w,wd,twallprof,twallprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
                            velprofd = _np.zeros((im+2*gh), order='F')
                            flinwall(w,wd,velprof,velprofd,'Jlo', gam, interf3, gh, im, jm)
                            # fluxprofd = _np.zeros((im+2*gh), order='F')
                            # flinwall(w,wd,fluxprof,fluxprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
        ...
        if rank ==0:
            ...
            # fillNPZ(filename, w, res, IA, JA, Jacvol, gh, twallprof)
            fillNPZ(filename, w, res, IA, JA, Jacvol, gh, velprof)
...
def dRdp_fromNPZtree(treename, out_dir = 'totodir'):
    ...
    # twallprofd = _np.zeros((im+2*gh), order='F')
    # twallprof  = _np.load(filename+".npz", allow_pickle=True)['wallprof']
    velprofd = _np.zeros((im+2*gh), order='F')
    velprof  = _np.zeros((im+2*gh), order='F')
    # fluxprofd = _np.zeros((im+2*gh), order='F')
    # fluxprof  = _np.zeros((im+2*gh), order='F')
    ...
    interf3      =  _np.zeros((2,2), order='F')
    # interf3[0,0] = 1-gh # imin
    interf3[0,0] = 1 + ilimit # imin
    interf3[0,1] = 1  # jmin
    # interf3[1,0] = im+gh # imax
    interf3[1,0] = im - ilimit # imax
    interf3[1,1] = 1  # jmax
    # twallprofd = _np.zeros(im-2*ilimit)
    # twallprof  = 4.395*_np.ones(im-2*ilimit)
    velprofd   = _np.zeros(im-2*ilimit)
    velprof    = _np.zeros(im-2*ilimit)
    # fluxprofd   = _np.zeros(im-2*ilimit)
    # fluxprof    = _np.zeros(im-2*ilimit)
    ...
    # for m in range(im):
    for m in range(ilimit, im-ilimit): 
        wd   = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
        # twallprofd *= 0.
        velprofd *= 0.
        # fluxprofd *= 0.
        # f_misc.testvector_twall_short(twallprofd,m,ilimit,im-ilimit-1)
        f_misc.testvector_twall_short(velprofd,m,ilimit,im-ilimit-1)
        # f_misc.testvector_twall_short(fluxprofd,m,ilimit,im-ilimit-1)
    ...
    print('Write dRdp...', flush=True)
    fillNPZ_dRdp(filename, IAdp, JAdp, Jacdp)
    print("...written")

def dQchudq_fromNPZtree(treename, fileforc, fileresp, out_dir = 'totodir'):

    
    fprodchu = eval("f_chu.chuproduct_2d")
    flinprodchu = eval("f_linchu.chuproduct_2d_d")
    ...
    wforcr = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    wforcr[gh:-gh,gh:-gh,0] = ro
    wforcr[gh:-gh,gh:-gh,1] = rou 
    wforcr[gh:-gh,gh:-gh,2] = rov 
    wforcr[gh:-gh,gh:-gh,3] = row 
    wforcr[gh:-gh,gh:-gh,4] = roe 
    wforci = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    wforci[gh:-gh,gh:-gh,0] = roi
    wforci[gh:-gh,gh:-gh,1] = roui
    wforci[gh:-gh,gh:-gh,2] = rovi
    wforci[gh:-gh,gh:-gh,3] = rowi
    wforci[gh:-gh,gh:-gh,4] = roei

    wd   = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    prod = _np.zeros((im, jm,5), order='F')
    proddr = _np.zeros((im, jm,5), order='F')
    proddi = _np.zeros((im, jm,5), order='F')
    proddfr = _np.zeros((im, jm,5), order='F')
    proddfi = _np.zeros((im, jm,5), order='F')
    ...
    IAdz   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    JAdz   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    Jachfr  = _np.zeros((nbentry), order='F')
    Jachfi  = _np.zeros((nbentry), order='F')
    Jacdfz  = _np.zeros((nbentry), order='F')
    IAdfz   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    JAdfz   = _np.zeros((nbentry), dtype=_np.int32, order='F')

    for m in range(5):
        wd *= 0.
        f_misc.testvector_dchu(wd,m,gh,im,jm)

        flinprodchu(prod, proddr, w, wd, wmoder[gh:-gh,gh:-gh,:], vol, gh, mach, cv, gam, rgaz, cs, muref, tref, cs, im, jm)

        wd *= 0.
        f_misc.testvector_dchu(wd,m,gh,im,jm)

        flinprodchu(prod, proddi, w, wd, wmodei[gh:-gh,gh:-gh,:], vol, gh, mach, cv, gam, rgaz, cs, muref, tref, cs, im, jm) 

        wd *= 0.
        f_misc.testvector_dchu(wd,m,gh,im,jm)

        flinprodchu(prod, proddfr, w, wd, wforcr[gh:-gh,gh:-gh,:], vol, gh, mach, cv, gam, rgaz, cs, muref, tref, cs, im, jm)

        wd *= 0.
        f_misc.testvector_dchu(wd,m,gh,im,jm)

        flinprodchu(prod, proddfi, w, wd, wforci[gh:-gh,gh:-gh,:], vol, gh, mach, cv, gam, rgaz, cs, muref, tref, cs, im, jm)     

        f_misc.computederivativechufromjv(Jachr,IAdz,JAdz,proddr,m,gh,im,jm)
        f_misc.computederivativechufromjv(Jachi,IAdz,JAdz,proddi,m,gh,im,jm)
        f_misc.computederivativechufromjv(Jachfr,IAdfz,JAdfz,proddfr,m,gh,im,jm)
        f_misc.computederivativechufromjv(Jachfi,IAdfz,JAdfz,proddfi,m,gh,im,jm)

    Jacdz = Jachr + 1.j*Jachi
    Jacdfz = Jachfr + 1.j*Jachfi

    mini = 2.e-16
    IAdz, JAdz, Jacdz = remove_zero_jac(IAdz, JAdz, Jacdz, mini)
    IAdfz, JAdfz, Jacdfz = remove_zero_jac(IAdfz, JAdfz, Jacdfz, mini)

    print("Write dQchudq...", flush=True)

    dic = dict(_np.load(filename+'.npz', allow_pickle=True))
    dic['IAdChuqdq'] = IAdz
    dic['JAdChuqdq'] = JAdz
    dic['AijdChuqdq'] = Jacdz
    dic['IAdChufdq'] = IAdfz
    dic['JAdChufdq'] = JAdfz
    dic['AijdChufdq'] = Jacdfz
    _np.savez(filename+'.npz', **dic)

    print("...written")


def dAdq_fromNPZtree(treename, fileresp, out_dir = 'totodir'):
    ...
    wmoder[gh:-gh,gh:-gh,3] = row 
    wmoder[gh:-gh,gh:-gh,4] = roe 
    flininflow(w, wmoder,'Ilo',interf1,field,nx,ny,gam,im,jm)
    flinnoref(w, wmoder, wbd,'Jhi',interf4,nx,ny,gam,gh,im,jm)
    flinoutflow(w, wmoder,'Ihi', interf2, im, jm, gh)
    # flinwall(w,wmoder,twallprof,twallprofd,'Jlo', gam,  rgaz, interf3, gh, im, jm)
    flinwall(w,wmoder,velprof,velprofd,'Jlo', gam, interf3, gh, im, jm)
    # flinwall(w,wmoder,fluxprof,fluxprofd,'Jlo', gam,  rgaz, interf3, gh, im, jm)
    ...
    wmodei[gh:-gh,gh:-gh,3] = rowi
    wmodei[gh:-gh,gh:-gh,4] = roei
    flininflow(w, wmodei,'Ilo',interf1,field,nx,ny,gam,im,jm)
    flinnoref(w, wmodei,wbd,'Jhi',interf4,nx,ny,gam,gh,im,jm)
    flinoutflow(w, wmodei,'Ihi', interf2, im, jm, gh)   
    # flinwall(w,wmodei,twallprof,twallprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
    flinwall(w,wmodei,velprof,velprofd,'Jlo', gam, interf3, gh, im, jm) 
    # flinwall(w,wmodei,fluxprof,fluxprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
    ...
    for m in range(5):
        for l in range(1 + 2*gh):
            for k in range(1 + 2*gh):
                ...
                flininflow(w, wdr,'Ilo',interf1,field,nx,ny,gam,im,jm)        
                wd[:gh,:,:] = wdr[:gh,:,:]
                finflow(w,'Ilo',interf1,field,nx,ny,gam,im,jm) 

                wdr = _np.copy(wd, order='F') 
                flinnoref(w, wdr,wbd,'Jhi',interf4,nx,ny,gam,gh,im,jm)
                wd[:,-gh:,:] = wdr[:,-gh:,:]
                fnoref(w,wbd,'Jhi',interf4,nx,ny,gam,gh,im,jm)  
                ...
                wdr = _np.copy(wd, order='F')
                # flinwall(w,wdr,twallprof,twallprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
                flinwall(w,wdr,velprof,velprofd,'Jlo', gam, interf3, gh, im, jm)
                # flinwall(w,wdr,fluxprof,fluxprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
                wd[:,:gh,:] = wdr[:,:gh,:]
                ...
                f_misc.computejacobianfromjv_dbyvol(Jachr,IAh,JAh,dhr,m,l,k,gh,im,jm,vol)
                f_misc.computejacobianfromjv_dbyvol(Jachi,IAh,JAh,dhi,m,l,k,gh,im,jm,vol)
            

    Jach = Jachr + 1.j*Jachi

    mini = 2.e-16
    IAh, JAh, Jach = remove_zero_jac(IAh, JAh, Jach, mini)

    print("Write dAdq...", flush=True)
...
def dAdq3D_fromNPZtree(treename, fileresp, out_dir = 'totodir'):
    ...
    wd   = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzr  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzi  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2r  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2i  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzrb  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzib  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2rb  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2ib  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    resd = _np.zeros((im+2*gh, jm+2*gh,5), order='F')

    nbentry = im*jm * (2*gh+1)*(2*gh+1) * 5*5
    Jacdz  = _np.zeros((nbentry), dtype=_np.complex128, order='F')
    Jacdzr = _np.zeros((nbentry), order='F')
    Jacdzi = _np.zeros((nbentry), order='F')
    Jacdz2  = _np.zeros((nbentry), dtype=_np.complex128, order='F')
    Jacdz2r = _np.zeros((nbentry), order='F')
    Jacdz2i = _np.zeros((nbentry), order='F')
    IAdz   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    JAdz   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    IAdz2   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    JAdz2   = _np.zeros((nbentry), dtype=_np.int32, order='F')
    ...
    for m in range(5):
        for l in range(1 + 2*gh):
            for k in range(1 + 2*gh):
                ...
                f_lindz.coeffs_5p_dz_d(resd, dzr, w, wd, wmoder, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
                f_lindz.coeffs_5p_dz2_d(resd, dz2r, w, wd, wmoder, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
                ...
                f_lindz.coeffs_5p_dz_d(resd, dzi, w, wd, wmodei, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
                f_lindz.coeffs_5p_dz2_d(resd, dz2i, w, wd, wmodei, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)  

            
                f_misc.computejacobianfromdz(Jacdzr,IAdz,JAdz,dzr,m,l,k,gh,im,jm)
                f_misc.computejacobianfromdz(Jacdzi,IAdz,JAdz,dzi,m,l,k,gh,im,jm)
                f_misc.computejacobianfromdz(Jacdz2r,IAdz2,JAdz2,dz2r,m,l,k,gh,im,jm)
                f_misc.computejacobianfromdz(Jacdz2i,IAdz2,JAdz2,dz2i,m,l,k,gh,im,jm)
            

    Jacdz = Jacdzr + 1.j*Jacdzi
    Jacdz2 = Jacdz2r + 1.j*Jacdz2i

    mini = 2.e-16
    IAdz, JAdz, Jacdz = remove_zero_jac(IAdz, JAdz, Jacdz, mini)
    IAdz2, JAdz2, Jacdz2 = remove_zero_jac(IAdz2, JAdz2, Jacdz2, mini)

    print("Write dAdq 3D...", flush=True)
    ...
    dic['IAdAdqdz2'] = IAdz2
    dic['JAdAdqdz2'] = JAdz2
    dic['AijdAdqdz2'] = Jacdz2
    _np.savez(filename+'.npz', **dic)

    print("...written")


def dAdp_fromNPZtree(treename, fileresp, out_dir = 'totodir'):
    ...
    # twallprofd = _np.zeros((im+2*gh), order='F')
    # twallprofd0 = _np.zeros((im+2*gh), order='F')
    # twallprof  = _np.load(filename+".npz", allow_pickle=True)['wallprof']
    velprofd = _np.zeros((im+2*gh), order='F')
    velprofd0 = _np.zeros((im+2*gh), order='F')
    velprof  = _np.zeros((im+2*gh), order='F')
    # fluxprofd = _np.zeros((im+2*gh), order='F')
    # fluxprofd0 = _np.zeros((im+2*gh), order='F')
    # fluxprof  = _np.zeros((im+2*gh), order='F')
    ...
    # twallprofd  = _np.zeros(im-2*ilimit)
    # twallprofd0 = _np.zeros(im-2*ilimit)
    # twallprof   = 4.395*_np.ones(im-2*ilimit)
    velprofd  = _np.zeros(im-2*ilimit)
    velprofd0 = _np.zeros(im-2*ilimit)
    velprof   = _np.zeros(im-2*ilimit)
    # fluxprofd  = _np.zeros(im-2*ilimit)
    # fluxprofd0 = _np.zeros(im-2*ilimit)
    # fluxprof   = _np.zeros(im-2*ilimit)
    ...
    dhr2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dhi2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    ...
    for m in range(ilimit, im-ilimit): 
        ...
        # flin2wall(w,wmoder,wd0,wd,twallprof,twallprofd0,twallprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
        flin2wall(w,wmoder,wd0,wd,velprof,velprofd0,velprofd,'Jlo', gam, interf3, gh, im, jm)
        # flin2wall(w,wmoder,wd0,wd,fluxprof,fluxprofd0,fluxprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)

        flinsch(res, dhr2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, k2, k4, im, jm)
        dhr = dhr + dhr2
        ...
        # flin2wall(w,wmodei,wd0,wd,twallprof,twallprofd0,twallprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)
        flin2wall(w,wmodei,wd0,wd,velprof,velprofd0,velprofd,'Jlo', gam, interf3, gh, im, jm)
        # flin2wall(w,wmodei,wd0,wd,fluxprof,fluxprofd0,fluxprofd,'Jlo', gam, rgaz, interf3, gh, im, jm)

        flinsch(res, dhi2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, k2, k4, im, jm)
        dhi = dhi + dhi2


        f_misc.computejacobian_twall_restrict(Jachr,IAh,JAh,dhr,m,gh,im,jm,vol)
        f_misc.computejacobian_twall_restrict(Jachi,IAh,JAh,dhi,m,gh,im,jm,vol)
...
def dAdp3D_fromNPZtree(treename, fileresp, out_dir = 'totodir'):
    ...
    dz2rb  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2ib  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzr2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dzi2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2r2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    dz2i2  = _np.zeros((im+2*gh, jm+2*gh,5), order='F')
    resd = _np.zeros((im+2*gh, jm+2*gh,5), order='F')

    # nbentry = im*(2*gh+1)*5 * im
    nbentry = im*jm*5 * im
    Jacdz  = _np.zeros((nbentry), dtype=_np.complex128, order='F')
    Jacdzr = _np.zeros((nbentry), order='F')
    Jacdzi = _np.zeros((nbentry), order='F')
    ...
    for m in range(ilimit, im-ilimit): 
        ...
        f_dz.coeffs_5p_dz(dzr2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
        f_dz.coeffs_5p_dz2(dz2r2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
        dzr = dzr + dzr2
        dz2r = dz2r + dz2r2
        ...
        f_dz.coeffs_5p_dz(dzi2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
        f_dz.coeffs_5p_dz2(dz2i2, w, wd, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, tref, cs, im, jm)
        dzi = dzi + dzi2
        dz2i = dz2i + dz2i2


        # f_misc.computejacobian_twall_notdivbyvol(Jacdzr,IAdz,JAdz,dzr,m,gh,im,jm)
        # f_misc.computejacobian_twall_notdivbyvol(Jacdzi,IAdz,JAdz,dzi,m,gh,im,jm)
        # f_misc.computejacobian_twall_notdivbyvol(Jacdz2r,IAdz2,JAdz2,dz2r,m,gh,im,jm)
        # f_misc.computejacobian_twall_notdivbyvol(Jacdz2i,IAdz2,JAdz2,dz2i,m,gh,im,jm)
        f_misc.computejacobian_twall_restrict_notdivbyvol(Jacdzr,IAdz,JAdz,dzr,m,gh,im,jm)
        f_misc.computejacobian_twall_restrict_notdivbyvol(Jacdzi,IAdz,JAdz,dzi,m,gh,im,jm)
        f_misc.computejacobian_twall_restrict_notdivbyvol(Jacdz2r,IAdz2,JAdz2,dz2r,m,gh,im,jm)
        f_misc.computejacobian_twall_restrict_notdivbyvol(Jacdz2i,IAdz2,JAdz2,dz2i,m,gh,im,jm)
    ...
    print("Write dAdp 3D...", flush=True)
