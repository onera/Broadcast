...
!1st direction
      temp3 = abs2*abs28d/abs28
      temp2 = (abs2d-temp3)/abs28
      k_sensor1dd = (abs2dd-(abs28d*abs2d0+abs2*abs28dd-temp3*abs28d0)/&
&       abs28-temp2*abs28d0)/abs28
      k_sensor1d = temp2
      k_sensor1d0 = (abs2d0-abs2*abs28d0/abs28)/abs28
      k_sensor1 = abs2/abs28
      ...
      ELSE
        ...
        abs29 = -x11
      END IF
      temp3 = abs3*abs29d/abs29
      temp2 = (abs3d-temp3)/abs29
      k_sensor2dd = (abs3dd-(abs29d*abs3d0+abs3*abs29dd-temp3*abs29d0)/&
&       abs29-temp2*abs29d0)/abs29
      k_sensor2d = temp2
      k_sensor2d0 = (abs3d0-abs3*abs29d0/abs29)/abs29
      k_sensor2 = abs3/abs29
...
! 2nd direction
      temp3 = abs4*abs30d/abs30
      temp2 = (abs4d-temp3)/abs30
      k_sensor1dd = (abs4dd-(abs30d*abs4d0+abs4*abs30dd-temp3*abs30d0)/&
&       abs30-temp2*abs30d0)/abs30
      k_sensor1d = temp2
      k_sensor1d0 = (abs4d0-abs4*abs30d0/abs30)/abs30
      k_sensor1 = abs4/abs30
      ...
      ELSE
        ...
        abs31 = -x13
      END IF
      temp3 = abs5*abs31d/abs31
      temp2 = (abs5d-temp3)/abs31
      k_sensor2dd = (abs5dd-(abs31d*abs5d0+abs5*abs31dd-temp3*abs31d0)/&
&       abs31-temp2*abs31d0)/abs31
      k_sensor2d = temp2
      k_sensor2d0 = (abs5d0-abs5*abs31d0/abs31)/abs31
      k_sensor2 = abs5/abs31
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs8*abs32d/abs32
    temp2 = (abs8d-temp3)/abs32
    k_sensor1dd = (abs8dd-(abs32d*abs8d0+abs8*abs32dd-temp3*abs32d0)/&
&     abs32-temp2*abs32d0)/abs32
    k_sensor1d = temp2
    k_sensor1d0 = (abs8d0-abs8*abs32d0/abs32)/abs32
    k_sensor1 = abs8/abs32
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs33 = -x15
    END IF
    temp3 = abs9*abs33d/abs33
    temp2 = (abs9d-temp3)/abs33
    k_sensor2dd = (abs9dd-(abs33d*abs9d0+abs9*abs33dd-temp3*abs33d0)/&
&     abs33-temp2*abs33d0)/abs33
    k_sensor2d = temp2
    k_sensor2d0 = (abs9d0-abs9*abs33d0/abs33)/abs33
    k_sensor2 = abs9/abs33
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs10*abs34d/abs34
    temp2 = (abs10d-temp3)/abs34
    k_sensor1dd = (abs10dd-(abs34d*abs10d0+abs10*abs34dd-temp3*abs34d0)/&
&     abs34-temp2*abs34d0)/abs34
    k_sensor1d = temp2
    k_sensor1d0 = (abs10d0-abs10*abs34d0/abs34)/abs34
    k_sensor1 = abs10/abs34
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs35 = -x17
    END IF
    temp3 = abs11*abs35d/abs35
    temp2 = (abs11d-temp3)/abs35
    k_sensor2dd = (abs11dd-(abs35d*abs11d0+abs11*abs35dd-temp3*abs35d0)/&
&     abs35-temp2*abs35d0)/abs35
    k_sensor2d = temp2
    k_sensor2d0 = (abs11d0-abs11*abs35d0/abs35)/abs35
    k_sensor2 = abs11/abs35
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs20*abs40d/abs40
    temp2 = (abs20d-temp3)/abs40
    k_sensor1dd = (abs20dd-(abs40d*abs20d0+abs20*abs40dd-temp3*abs40d0)/&
&     abs40-temp2*abs40d0)/abs40
    k_sensor1d = temp2
    k_sensor1d0 = (abs20d0-abs20*abs40d0/abs40)/abs40
    k_sensor1 = abs20/abs40
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs41 = -x23
    END IF
    temp3 = abs21*abs41d/abs41
    temp2 = (abs21d-temp3)/abs41
    k_sensor2dd = (abs21dd-(abs41d*abs21d0+abs21*abs41dd-temp3*abs41d0)/&
&     abs41-temp2*abs41d0)/abs41
    k_sensor2d = temp2
    k_sensor2d0 = (abs21d0-abs21*abs41d0/abs41)/abs41
    k_sensor2 = abs21/abs41
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs22*abs42d/abs42
    temp2 = (abs22d-temp3)/abs42
    k_sensor1dd = (abs22dd-(abs42d*abs22d0+abs22*abs42dd-temp3*abs42d0)/&
&     abs42-temp2*abs42d0)/abs42
    k_sensor1d = temp2
    k_sensor1d0 = (abs22d0-abs22*abs42d0/abs42)/abs42
    k_sensor1 = abs22/abs42
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs43 = -x25
    END IF
    temp3 = abs23*abs43d/abs43
    temp2 = (abs23d-temp3)/abs43
    k_sensor2dd = (abs23dd-(abs43d*abs23d0+abs23*abs43dd-temp3*abs43d0)/&
&     abs43-temp2*abs43d0)/abs43
    k_sensor2d = temp2
    k_sensor2d0 = (abs23d0-abs23*abs43d0/abs43)/abs43
    k_sensor2 = abs23/abs43
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs25dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs25d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs25d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs25 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs25dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs25d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs25d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs25 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs25*abs44d/abs44
    temp2 = (abs25d-temp3)/abs44
    k_sensor1dd = (abs25dd-(abs44d*abs25d0+abs25*abs44dd-temp3*abs44d0)/&
&     abs44-temp2*abs44d0)/abs44
    k_sensor1d = temp2
    k_sensor1d0 = (abs25d0-abs25*abs44d0/abs44)/abs44
    k_sensor1 = abs25/abs44
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs26dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs26d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs26d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs26 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs26dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs26d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs26d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs26 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs45 = -x27
    END IF
    temp3 = abs26*abs45d/abs45
    temp2 = (abs26d-temp3)/abs45
    k_sensor2dd = (abs26dd-(abs45d*abs26d0+abs26*abs45dd-temp3*abs45d0)/&
&     abs45-temp2*abs45d0)/abs45
    k_sensor2d = temp2
    k_sensor2d0 = (abs26d0-abs26*abs45d0/abs45)/abs45
    k_sensor2 = abs26/abs45
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
