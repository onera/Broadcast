...
!
!  Differentiation of bc_supandsubinlet_forhessian_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceD
!eadInstrs with!StaticTaping):
!   variations   of useful results: woutd
!   with respect to varying inputs: w
!   RW status of diff variables: w:in woutd:out wout:(loc)
!        Generated by TAPENADE     (INRIA, Ecuador team)
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of bc_supandsubinlet_forhessian_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDea
!dInstrs with!StaticTaping):
!   variations   of useful results: wout
!   with respect to varying inputs: w
!   RW status of diff variables: w:in wout:out
SUBROUTINE BC_SUPANDSUBINLET_FORHESSIAN_2D_D_D(wout, woutd0, woutd, &
& woutdd, w, wd0, wd, loc, interf, field, nx, ny, gam, im, jm, lm, gh)
  IMPLICIT NONE
...
! Returned objects ------------------------------------------------
  ...
  REAL*8, DIMENSION(1-gh:im+gh, 1-gh:jm+gh, 5), INTENT(INOUT) :: woutd0
...
! Non-required arguments -------------------------------------------
  ...
  REAL*8 :: rodd0, udd0, vdd0, wdid0, pdd0, utdd0, vtdd0, vndd0
  REAL*8 :: rodd, udd, vdd, wdid, pdd, utdd, vtdd, vndd
  REAL*8 :: roddd, uddd, vddd, wdidd, pddd, utddd, vtddd, vnddd
...
!
    ...
    DO de=1,gh
! dirichlet
      woutdd(i-de*i0, j-de*j0, 1) = 0.0_8
      woutd(i-de*i0, j-de*j0, 1) = 0.0_8
      woutd0(i-de*i0, j-de*j0, 1) = 0.0_8
      wout(i-de*i0, j-de*j0, 1) = field(l, de, 1)
      woutdd(i-de*i0, j-de*j0, 2) = 0.0_8
      woutd(i-de*i0, j-de*j0, 2) = 0.0_8
      woutd0(i-de*i0, j-de*j0, 2) = 0.0_8
      wout(i-de*i0, j-de*j0, 2) = field(l, de, 2)
      woutdd(i-de*i0, j-de*j0, 3) = 0.0_8
      woutd(i-de*i0, j-de*j0, 3) = 0.0_8
      woutd0(i-de*i0, j-de*j0, 3) = 0.0_8
      wout(i-de*i0, j-de*j0, 3) = field(l, de, 3)
      woutdd(i-de*i0, j-de*j0, 4) = 0.0_8
      woutd(i-de*i0, j-de*j0, 4) = 0.0_8
      woutd0(i-de*i0, j-de*j0, 4) = 0.0_8
      wout(i-de*i0, j-de*j0, 4) = field(l, de, 4)
      woutdd(i-de*i0, j-de*j0, 5) = 0.0_8
      woutd(i-de*i0, j-de*j0, 5) = 0.0_8
      woutd0(i-de*i0, j-de*j0, 5) = 0.0_8
      wout(i-de*i0, j-de*j0, 5) = field(l, de, 5)
      da = de - 1
      IF (mach .LT. one) THEN
! l-state
        roddd = woutdd(i-de*i0, j-de*j0, 1)
        rodd = woutd(i-de*i0, j-de*j0, 1)
        rodd0 = woutd0(i-de*i0, j-de*j0, 1)
        ...
        rom1 = one/rod
        tempd = woutd0(i-de*i0, j-de*j0, 2)
        temp = wout(i-de*i0, j-de*j0, 2)
        temp0 = woutd(i-de*i0, j-de*j0, 2)
        uddd = temp0*rom1d0 + rom1*woutdd(i-de*i0, j-de*j0, 2) + rom1d*&
&         tempd + temp*rom1dd
        udd = rom1*temp0 + temp*rom1d
        udd0 = rom1*tempd + temp*rom1d0
        ud = temp*rom1
        tempd = woutd0(i-de*i0, j-de*j0, 3)
        temp = wout(i-de*i0, j-de*j0, 3)
        temp0 = woutd(i-de*i0, j-de*j0, 3)
        vddd = temp0*rom1d0 + rom1*woutdd(i-de*i0, j-de*j0, 3) + rom1d*&
&         tempd + temp*rom1dd
        vdd = rom1*temp0 + temp*rom1d
        vdd0 = rom1*tempd + temp*rom1d0
        vd = temp*rom1
        tempd = woutd0(i-de*i0, j-de*j0, 4)
        temp = wout(i-de*i0, j-de*j0, 4)
        temp0 = woutd(i-de*i0, j-de*j0, 4)
        wdidd = temp0*rom1d0 + rom1*woutdd(i-de*i0, j-de*j0, 4) + rom1d*&
&         tempd + temp*rom1dd
        wdid = rom1*temp0 + temp*rom1d
        wdid0 = rom1*tempd + temp*rom1d0
        wdi = temp*rom1
        tempd = 2*ud*udd0 + 2*vd*vdd0 + 2*wdi*wdid0
        temp = ud*ud + vd*vd + wdi*wdi
        temp0 = 2*ud*udd + 2*vd*vdd + 2*wdi*wdid
        pddd = gam1*(woutdd(i-de*i0, j-de*j0, 5)-half*(rodd*tempd+temp*&
&         roddd+temp0*rodd0+rod*(udd*2*udd0+2*ud*uddd+vdd*2*vdd0+2*vd*&
&         vddd+wdid*2*wdid0+2*wdi*wdidd)))
        pdd = gam1*(woutd(i-de*i0, j-de*j0, 5)-half*(temp*rodd+rod*temp0&
&         ))
        pdd0 = gam1*(woutd0(i-de*i0, j-de*j0, 5)-half*(temp*rodd0+rod*&
&         tempd))
        pd = gam1*(wout(i-de*i0, j-de*j0, 5)-half*(rod*temp))
        ...
        vtd = vd - vnd*nynorm
        tempd = woutd0(i-de*i0, j-de*j0, 5) + pdd0
        temp = wout(i-de*i0, j-de*j0, 5) + pd
        temp0 = woutd(i-de*i0, j-de*j0, 5) + pdd
        hldd = temp0*rom1d0 + rom1*(woutdd(i-de*i0, j-de*j0, 5)+pddd) + &
&         rom1d*tempd + temp*rom1dd
        hld = rom1*temp0 + temp*rom1d
        hld0 = rom1*tempd + temp*rom1d0
        hl = temp*rom1
! r-state
        rosdd = woutdd(i-da*i0, j-da*j0, 1)
        rosd = woutd(i-da*i0, j-da*j0, 1)
        rosd0 = woutd0(i-da*i0, j-da*j0, 1)
        ...
        rosm1 = one/ros
        tempd = woutd0(i-da*i0, j-da*j0, 2)
        temp = wout(i-da*i0, j-da*j0, 2)
        temp0 = woutd(i-da*i0, j-da*j0, 2)
        usdd = temp0*rosm1d0 + rosm1*woutdd(i-da*i0, j-da*j0, 2) + &
&         rosm1d*tempd + temp*rosm1dd
        usd = rosm1*temp0 + temp*rosm1d
        usd0 = rosm1*tempd + temp*rosm1d0
        us = temp*rosm1
        tempd = woutd0(i-da*i0, j-da*j0, 3)
        temp = wout(i-da*i0, j-da*j0, 3)
        temp0 = woutd(i-da*i0, j-da*j0, 3)
        vsdd = temp0*rosm1d0 + rosm1*woutdd(i-da*i0, j-da*j0, 3) + &
&         rosm1d*tempd + temp*rosm1dd
        vsd = rosm1*temp0 + temp*rosm1d
        vsd0 = rosm1*tempd + temp*rosm1d0
        vs = temp*rosm1
        tempd = woutd0(i-da*i0, j-da*j0, 4)
        temp = wout(i-da*i0, j-da*j0, 4)
        temp0 = woutd(i-da*i0, j-da*j0, 4)
        wsdd = temp0*rosm1d0 + rosm1*woutdd(i-da*i0, j-da*j0, 4) + &
&         rosm1d*tempd + temp*rosm1dd
        ...
        psdd = gam1*(woutdd(i-da*i0, j-da*j0, 5)-half*(rosd*tempd+temp*&
&         rosdd+temp0*rosd0+ros*(usd*2*usd0+2*us*usdd+vsd*2*vsd0+2*vs*&
&         vsdd+wsd*2*wsd0+2*ws*wsdd)))
        psd = gam1*(woutd(i-da*i0, j-da*j0, 5)-half*(temp*rosd+ros*temp0&
&         ))
        psd0 = gam1*(woutd0(i-da*i0, j-da*j0, 5)-half*(temp*rosd0+ros*&
&         tempd))
        ps = gam1*(wout(i-da*i0, j-da*j0, 5)-half*(ros*temp))
        ...
        vts = vs - vns*nynorm
        tempd = woutd0(i-da*i0, j-da*j0, 5) + psd0
        temp = wout(i-da*i0, j-da*j0, 5) + ps
        temp0 = woutd(i-da*i0, j-da*j0, 5) + psd
        hrdd = temp0*rosm1d0 + rosm1*(woutdd(i-da*i0, j-da*j0, 5)+psdd) &
&         + rosm1d*tempd + temp*rosm1dd
...
&         oneonrplusoned0
        ...
        temp0 = r*wsd + ws*rd + wdid
        wwdd = temp0*oneonrplusoned0 + oneonrplusone*(wsd*rd0+r*wsdd+rd*&
&         wsd0+ws*rdd+wdidd) + oneonrplusoned*(r*wsd0+ws*rd0+wdid0) + (&
&         ws*r+wdi)*oneonrplusonedd
        wwd = oneonrplusone*temp0 + (ws*r+wdi)*oneonrplusoned
        wwd0 = oneonrplusone*(r*wsd0+ws*rd0+wdid0) + (ws*r+wdi)*&
&         oneonrplusoned0
...
&         nynorm*vnd0)*rod1 + ro*(vtd0d+nynorm*vnd0d)
        ...
        rov = ro*(vt+vn*nynorm)
        row = field(j, de, 4)
        woutdd(i-de*i0, j-de*j0, 1) = rod0d
        woutd(i-de*i0, j-de*j0, 1) = rod0
        woutd0(i-de*i0, j-de*j0, 1) = rod1
        wout(i-de*i0, j-de*j0, 1) = ro
        woutdd(i-de*i0, j-de*j0, 2) = roudd
        woutd(i-de*i0, j-de*j0, 2) = roud
        woutd0(i-de*i0, j-de*j0, 2) = roud0
        wout(i-de*i0, j-de*j0, 2) = rou
        woutdd(i-de*i0, j-de*j0, 3) = rovdd
        woutd(i-de*i0, j-de*j0, 3) = rovd
        woutd0(i-de*i0, j-de*j0, 3) = rovd0
        ...
        woutdd(i-de*i0, j-de*j0, 5) = roedd + half*(roud*2*roud0+2*rou*&
&         roudd+rovd*2*rovd0+2*rov*rovdd-rod0*tempd-temp*rod0d-temp1*&
&         rod1)/ro
        woutd(i-de*i0, j-de*j0, 5) = roed + half*temp1
        woutd0(i-de*i0, j-de*j0, 5) = roed0 + half*tempd
        wout(i-de*i0, j-de*j0, 5) = roe + half*temp
      END IF
    END DO
  END DO
END SUBROUTINE BC_SUPANDSUBINLET_FORHESSIAN_2D_D_D
