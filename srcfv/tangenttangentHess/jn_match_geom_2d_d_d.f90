...
!
!  Differentiation of jn_match_geom_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs with!
!StaticTaping):
...
!                              Join match 2-D
! =============================================================================
SUBROUTINE JN_MATCH_GEOM_2D_D_D(wr, wrd, wrdd, prr, gh1r, gh2r, gh3r, &
& gh4r, imr, jmr, wd, wdd, wddd, prd, gh1d, gh2d, gh3d, gh4d, imd, jmd, &
& tr)
  IMPLICIT NONE
!
...
! Input variables -------------------------------------------------
  ...
  REAL*8, DIMENSION(1-gh1d:imd+gh2d, 1-gh3d:jmd+gh4d), INTENT(IN) :: &
& wddd
...
!
  DO j=j1,j2,jstep
    DO i=i1,i2,istep
      ir = prr(1, 1) + idd - 1
      jr = prr(1, 2) + jdd - 1
      wrdd(ir, jr) = wddd(ind1, ind2)
      idd = idd + 1
    END DO
    idd = 1
    jdd = jdd + 1
  END DO
END SUBROUTINE JN_MATCH_GEOM_2D_D_D
