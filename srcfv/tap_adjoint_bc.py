...
for file in files:
    if 'rbc' in file:
        ...
        print(ext)
        filein  = dir + file
        tapfile = tapdir + file1 + '_b' + ext 
        if 'extrap' in file:
            for order in [2,3,4,5,7,9]:
                routine = 'bc_extrapolate_o%i_2d' % order
                exec_str = 'tapenade %s -head "%s(w)/(w)" -b -optim diffliveness  -optim statictape -O %s' % (filein, routine, tapdir)
                print(exec_str)
                os.system(exec_str)
                os.system('mv %s %s' % (tapfile, tapdir + file1 + 'o%i_b%s' % (order, ext)))
        else:
            if 'sa' in file:
                if 'no' in file:
            ...
            if 'hess' in file:
                exec_str = 'tapenade %s -head "%s(wout)/(w)" -b -optim diffliveness  -optim statictape -O %s' % (filein, routine, tapdir)
            else:
                exec_str = 'tapenade %s -head "%s(w)/(w)" -b -optim diffliveness  -optim statictape -O %s' % (filein, routine, tapdir)
            print(exec_str)
            os.system(exec_str)
