...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC5_SPONGE_DOWN_2D_D(residu, residud, w, wd, x0, y0&
& , nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref&
& , tref, s_suth, k2, k4, im, jm, lmax, r2, r3, r4, sourcear, sourceard)
  IMPLICIT NONE
...
!! Sponge layer downstream
      IF (xc(i, j) - (xc(im, j)-lmax) .LT. zero) THEN
        ralpha = zero
      ELSE
        pwx1 = (xc(i, j)-(xc(im, j)-lmax))/lmax
        pwr1 = pwx1**r3
        ralpha = r2*pwr1
      END IF
      wmoyd(:) = wd(i, j, :) - ralpha*(r0*3.d0*wd(i, j, :)+r1*(wd(i+1, j&
&       , :)+wd(i-1, j, :)+wd(i, j+1, :)+wd(i, j-1, :)))
      temp0 = r4*ralpha*vol(i, j)
      sourced(:) = temp0*(wd(i, j, :)-wmoyd(:))
      sourceard(i, j, :) = sourced(:)
      sc1 = nx(i, j, 1)
      sc2 = ny(i, j, 1)
...
!DIR$ IVDEP
    DO i=1,im
      residud(i, j, 1) = hnd(i, j, 1, 1) - hnd(i+1, j, 1, 1) - sourceard&
&       (i, j, 1) + hnd(i, j, 1, 2) - hnd(i, j+1, 1, 2)
      residud(i, j, 2) = hnd(i, j, 2, 1) - hnd(i+1, j, 2, 1) - sourceard&
&       (i, j, 2) + hnd(i, j, 2, 2) - hnd(i, j+1, 2, 2)
      residud(i, j, 3) = hnd(i, j, 3, 1) - hnd(i+1, j, 3, 1) - sourceard&
&       (i, j, 3) + hnd(i, j, 3, 2) - hnd(i, j+1, 3, 2)
      residud(i, j, 4) = hnd(i, j, 4, 1) - hnd(i+1, j, 4, 1) - sourceard&
&       (i, j, 4) + hnd(i, j, 4, 2) - hnd(i, j+1, 4, 2)
      residud(i, j, 5) = hnd(i, j, 5, 1) - hnd(i+1, j, 5, 1) - sourceard&
&       (i, j, 5) + hnd(i, j, 5, 2) - hnd(i, j+1, 5, 2)
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC5_SPONGE_DOWN_2D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
