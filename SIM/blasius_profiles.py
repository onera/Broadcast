...
## Small changes to compute Blasius self-similar profiles (incompressible)
...
def blasius_profiles(xc, yc, Mach, dphys, prandtl=0.72, gam=1.4, isplot=False, damped=False):
    ...
    if os.path.isfile('simfunctions_mach_%s_eps_%s_max_%s' % (Mach,eps,infini)):
        ...
        tol     = 1.e-6
        itermax = 30
        eta, f, df, g = BL.similarity(Mach,prandtl,gam,eps,infini,tol,itermax)
    ...
    for i in range(Nx):
        ...
        for j in range(1,Ny):
            ...
            eta_real = Y[j] / X[i] * ( uinf*X[i]*rhoinf/(2.*muinf) )**0.5
            ...
            v[i,j]   = _np.sqrt(muinf/(2.*X[i]*uinf*rhoinf)) * uinf * (eta[ind_sol] * df[ind_sol] - f[ind_sol])
...
if __name__ == '__main__':
    ...
    rho, u, v, E = blasius_profiles(X, Y, Mach, dphys, prandtl)
