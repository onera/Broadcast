...
!
!  Differentiation of bc_wall_viscous_iso_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs
! with!StaticTaping):
!   variations   of useful results: w wd
!   with respect to varying inputs: w twall
!   RW status of diff variables: w:in-out twall:in wd:out
!        Generated by TAPENADE     (INRIA, Ecuador team)
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of bc_wall_viscous_iso_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs w
!ith!StaticTaping):
!   variations   of useful results: w
!   with respect to varying inputs: w
!   RW status of diff variables: w:in-out
SUBROUTINE BC_WALL_VISCOUS_ISO_2D_D_D(w, wd0, wd, wdd, twall, twalld, &
& loc, gam, rgaz, interf, gh, im, jm)
  IMPLICIT NONE
!
!
...
!! ???
! roi = pw / (rgaz * twall)
    temp0 = pwd/(rgaz*twall)
    roidd = two*(pwdd-temp0*rgaz*twalld)/(rgaz*twall) - roedd
    roid = two*temp0 - roed
    temp0 = pw/(rgaz*twall)
    roid0 = two*(pwd0-temp0*rgaz*twalld)/(rgaz*twall) - roed0
    roi = two*temp0 - roe
! pi  = pw
...
! roiei = w(i+de*i0,j+de*j0,5)
    ...
    END DO
  END DO
END SUBROUTINE BC_WALL_VISCOUS_ISO_2D_D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
