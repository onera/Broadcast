...
!
!  Differentiation of bc_wall_viscous_iso_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs w
!ith!StaticTaping):
!   gradient     of useful results: w twall rgaz gam
!   with respect to varying inputs: w twall rgaz gam
!   RW status of diff variables: w:in-out twall:incr rgaz:incr
!                gam:incr
...
SUBROUTINE BC_WALL_VISCOUS_ISO_2D_B(w, wb, twall, twallb, loc, gam, gamb&
& , rgaz, rgazb, interf, gh, im, jm)
  IMPLICIT NONE
!
!
...
&       , j-de*j0, 2) + wb(i-de*i0, j-de*j0, 1)
    ...
    END DO
    tempb = two*roib/(rgaz*twall)
    pib = roieib/gam1
    pwb = four*third*pib + tempb
    peb = ct0*pwb - third*pib
    tempb0 = -(pw*tempb/(rgaz*twall))
    pe1b = ct1*pwb
    tempb = gam1*pe1b
...
&     /gam1**2 + (w(i, j, 5)-half*(roe*ve2))*peb
    rgazb = rgazb + twall*tempb0
    twallb = twallb + rgaz*tempb0
  ...
  END DO
  gamb = gamb + gam1b
END SUBROUTINE BC_WALL_VISCOUS_ISO_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
