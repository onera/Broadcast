...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs8*abs20d/abs20
    temp2 = (abs8d-temp3)/abs20
    k_sensor1dd = (abs8dd-(abs20d*abs8d0+abs8*abs20dd-temp3*abs20d0)/&
&     abs20-temp2*abs20d0)/abs20
    k_sensor1d = temp2
    k_sensor1d0 = (abs8d0-abs8*abs20d0/abs20)/abs20
    k_sensor1 = abs8/abs20
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs21 = -x11
    END IF
    temp3 = abs9*abs21d/abs21
    temp2 = (abs9d-temp3)/abs21
    k_sensor2dd = (abs9dd-(abs21d*abs9d0+abs9*abs21dd-temp3*abs21d0)/&
&     abs21-temp2*abs21d0)/abs21
    k_sensor2d = temp2
    k_sensor2d0 = (abs9d0-abs9*abs21d0/abs21)/abs21
    k_sensor2 = abs9/abs21
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs10*abs22d/abs22
    temp2 = (abs10d-temp3)/abs22
    k_sensor1dd = (abs10dd-(abs22d*abs10d0+abs10*abs22dd-temp3*abs22d0)/&
&     abs22-temp2*abs22d0)/abs22
    k_sensor1d = temp2
    k_sensor1d0 = (abs10d0-abs10*abs22d0/abs22)/abs22
    k_sensor1 = abs10/abs22
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs23 = -x13
    END IF
    temp3 = abs11*abs23d/abs23
    temp2 = (abs11d-temp3)/abs23
    k_sensor2dd = (abs11dd-(abs23d*abs11d0+abs11*abs23dd-temp3*abs23d0)/&
&     abs23-temp2*abs23d0)/abs23
    k_sensor2d = temp2
    k_sensor2d0 = (abs11d0-abs11*abs23d0/abs23)/abs23
    k_sensor2 = abs11/abs23
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs13dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs13d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs13d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs13 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs13dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs13d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs13d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs13 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs13*abs24d/abs24
    temp2 = (abs13d-temp3)/abs24
    k_sensor1dd = (abs13dd-(abs24d*abs13d0+abs13*abs24dd-temp3*abs24d0)/&
&     abs24-temp2*abs24d0)/abs24
    k_sensor1d = temp2
    k_sensor1d0 = (abs13d0-abs13*abs24d0/abs24)/abs24
    k_sensor1 = abs13/abs24
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs14dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs14d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs14d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs14 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs14dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs14d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs14d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs14 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs25 = -x15
    END IF
    temp3 = abs14*abs25d/abs25
    temp2 = (abs14d-temp3)/abs25
    k_sensor2dd = (abs14dd-(abs25d*abs14d0+abs14*abs25dd-temp3*abs25d0)/&
&     abs25-temp2*abs25d0)/abs25
    k_sensor2d = temp2
    k_sensor2d0 = (abs14d0-abs14*abs25d0/abs25)/abs25
    k_sensor2 = abs14/abs25
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
