...
&     *dissro1b
    ...
    coefb = k2*eps2b
    max5b = max10*max15*coefb
    max10b = max5*max15*coefb
    max15b = max5*max10*coefb
...
&     *dissro2b
    ...
    END IF
    coefb = k2*eps2b
    max4b = max9*max14*coefb
    max9b = max4*max14*coefb
    max14b = max4*max9*coefb
...
&     *dissro1b
    ...
    coefb = k2*eps2b
    max3b = max8*max13*coefb
    max8b = max3*max13*coefb
    max13b = max3*max8*coefb
...
&       diffro+eps4*predro2)*dissro2b
      ...
      END IF
      coefb = k2*eps2b
      max2b = max7*max12*coefb
      max7b = max2*max12*coefb
      max12b = max2*max7*coefb
...
&       diffro+eps4*predro1)*dissro1b
      ...
      coefb = k2*eps2b
      max1b = max6*max11*coefb
      max6b = max1*max11*coefb
      max11b = max1*max6*coefb
...
&       , 2))
      ...
      divu = gradu(i, j, 1) + gradv(i, j, 2)
      temp0 = SQRT(nx2*c2r)
      temp = divu/(temp0+1.d-15)
      temp1 = 10.d0*vol(i, j)
      tempb = -(temp1*(1.0-TANH(temp1*temp+2.5d0)**2)*half*dxm1b/(temp0+&
&       1.d-15))
      divub = tempb
      IF (nx2*c2r .EQ. 0.0) THEN
        c2rb = 0.0_8
      ELSE
        c2rb = -(nx2*temp*tempb/(2.0*temp0))
      END IF
