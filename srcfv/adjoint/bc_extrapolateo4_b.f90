...
!
!  Differentiation of bc_extrapolate_o4_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs wit
!h!StaticTaping):
!   gradient     of useful results: w
!   with respect to varying inputs: w
!   RW status of diff variables: w:in-out
SUBROUTINE BC_EXTRAPOLATE_O4_2D_B(w, wb, loc, interf, im, jm, gh, em)
  IMPLICIT NONE
...
!   write(200,*) loc, 'jmax = ', interf(2,2)
  ...
  DO l=lmax,lmin,-1
    DO de=gh,1,-1
      ...
      wb(i-i0*d1, j-j0*d1, :) = wb(i-i0*d1, j-j0*d1, :) + four*tmpb
      wb(i-i0*d2, j-j0*d2, :) = wb(i-i0*d2, j-j0*d2, :) - six*tmpb
      wb(i-i0*d3, j-j0*d3, :) = wb(i-i0*d3, j-j0*d3, :) + four*tmpb
      wb(i-i0*d4, j-j0*d4, :) = wb(i-i0*d4, j-j0*d4, :) - tmpb
    END DO
    CALL POPINTEGER4(j)
    CALL POPINTEGER4(i)
  END DO
  CALL POPCONTROL2B(branch)
END SUBROUTINE BC_EXTRAPOLATE_O4_2D_B
