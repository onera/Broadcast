...
!
!  Differentiation of flux_num_dnc9_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs with!
!StaticTaping):
...
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of flux_num_dnc9_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs with!St
!aticTaping):
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC9_2D_D_D(residu, residud, residudd, w, wd0, wd, &
...
&     )
    ...
    lambda = mmu*cpprandtl
!
    fvrou1dd = twothird*(mmud*(two*uxd0-vyd0)+(two*ux-vy)*mmudd+(two*uxd&
&     -vyd)*mmud0+mmu*(two*uxdd-vydd))
    fvrou1d = twothird*((two*ux-vy)*mmud+mmu*(two*uxd-vyd))
    fvrou1d0 = twothird*((two*ux-vy)*mmud0+mmu*(two*uxd0-vyd0))
    fvrou1 = twothird*mmu*(two*ux-vy)
...
&     (uydd+vxdd)
    ...
    gvrou1d0 = (uy+vx)*mmud0 + mmu*(uyd0+vxd0)
    gvrou1 = mmu*(uy+vx)
    gvrov1dd = twothird*(mmud*(two*vyd0-uxd0)+(two*vy-ux)*mmudd+(two*vyd&
&     -uxd)*mmud0+mmu*(two*vydd-uxdd))
    gvrov1d = twothird*((two*vy-ux)*mmud+mmu*(two*vyd-uxd))
    gvrov1d0 = twothird*((two*vy-ux)*mmud0+mmu*(two*vyd0-uxd0))
    gvrov1 = twothird*mmu*(-ux+two*vy)
    gvrow1dd = mmud*wyd0 + wy*mmudd + wyd*mmud0 + mmu*wydd
...
&     )
    ...
    lambda = mmu*cpprandtl
!
    fvrou2dd = twothird*(mmud*(two*uxd0-vyd0)+(two*ux-vy)*mmudd+(two*uxd&
&     -vyd)*mmud0+mmu*(two*uxdd-vydd))
    fvrou2d = twothird*((two*ux-vy)*mmud+mmu*(two*uxd-vyd))
    fvrou2d0 = twothird*((two*ux-vy)*mmud0+mmu*(two*uxd0-vyd0))
    fvrou2 = twothird*mmu*(two*ux-vy)
...
&     (uydd+vxdd)
    ...
    gvrou2d0 = (uy+vx)*mmud0 + mmu*(uyd0+vxd0)
    gvrou2 = mmu*(uy+vx)
    gvrov2dd = twothird*(mmud*(two*vyd0-uxd0)+(two*vy-ux)*mmudd+(two*vyd&
&     -uxd)*mmud0+mmu*(two*vydd-uxdd))
    gvrov2d = twothird*((two*vy-ux)*mmud+mmu*(two*vyd-uxd))
    gvrov2d0 = twothird*((two*vy-ux)*mmud0+mmu*(two*vyd0-uxd0))
    gvrov2 = twothird*mmu*(-ux+two*vy)
...
&       , j, 5, 2) - hndd(i, j+1, 5, 2)
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC9_2D_D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
