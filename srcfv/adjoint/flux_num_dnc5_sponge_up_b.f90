...
!
!  Differentiation of flux_num_dnc5_sponge_up_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInst
!rs with!StaticTaping):
!   gradient     of useful results: residu
!   with respect to varying inputs: w residu
!   RW status of diff variables: w:out residu:in-out sourcear:(loc)
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC5_SPONGE_UP_2D_B(residu, residub, w, wb, x0, y0, &
& nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam, rgaz, cs, muref, &
& tref, s_suth, k2, k4, im, jm, lmax, r2, r3, r4, sourcear, sourcearb)
  IMPLICIT NONE
...
& gradmu
  ...
  REAL*8, DIMENSION(5) :: wmoyb, sourceb
  INTEGER :: i, j, h
...
!DIR$ IVDEP
    DO i=1,im
      ...
      sourcearb(i, j, 5) = sourcearb(i, j, 5) - residub(i, j, 5)
      ...
      sourcearb(i, j, 4) = sourcearb(i, j, 4) - residub(i, j, 4)
      ...
      sourcearb(i, j, 3) = sourcearb(i, j, 3) - residub(i, j, 3)
      ...
      sourcearb(i, j, 2) = sourcearb(i, j, 2) - residub(i, j, 2)
      ...
      sourcearb(i, j, 1) = sourcearb(i, j, 1) - residub(i, j, 1)
...
&     nx(i, j, 2)*volf(i, j, 2)*uxb
    ...
    hnb(i, j, 1, 1) = 0.0_8
    sourceb = 0.0_8
    sourceb(:) = sourcearb(i, j, :)
    sourcearb(i, j, :) = 0.0_8
    wmoyb = 0.0_8
    tempb2 = r4*ralpha*vol(i, j)*sourceb(:)
    wmoyb(:) = -tempb2
    wb(i, j, :) = wb(i, j, :) + tempb2 + (1.0-r0*3.d0*ralpha)*wmoyb
    tempb2 = -(r1*ralpha*wmoyb(:))
    wb(i+1, j, :) = wb(i+1, j, :) + tempb2
    wb(i-1, j, :) = wb(i-1, j, :) + tempb2
    wb(i, j+1, :) = wb(i, j+1, :) + tempb2
    wb(i, j-1, :) = wb(i, j-1, :) + tempb2
...
&       (i, j)
    ...
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC5_SPONGE_UP_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
