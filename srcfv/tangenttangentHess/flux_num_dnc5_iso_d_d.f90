...
!
!  Differentiation of flux_num_dnc5_iso_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadInstrs w
!ith!StaticTaping):
...
! =============================================================================
!
SUBROUTINE FLUX_NUM_DNC5_ISO_2D_D_D(residu, residud, residudd, w, wd0, &
& wd, twall, x0, y0, nx, ny, xc, yc, vol, volf, gh, cp, cv, prandtl, gam&
...
! vel_wall = 0 ==> dveli/dxi = 2 * velcenter_i *nxi/vol
    ...
    temp0 = nx(i, j, 2)*volf(i, j, 2)
    txdd = temp0*tlocdd(i, j)
    txd = temp0*tlocd(i, j)
    txd0 = temp0*tlocd0(i, j)
    tx = temp0*(tloc(i, j)-twall)
    temp0 = ny(i, j, 2)*volf(i, j, 2)
    tydd = temp0*tlocdd(i, j)
    tyd = temp0*tlocd(i, j)
    tyd0 = temp0*tlocd0(i, j)
...
&     i, j, 2)*gvrov2dd
    ...
    hndd(i, j, 5, 2) = -(nx(i, j, 2)*fvroe2dd) - ny(i, j, 2)*gvroe2dd
  END DO
  residudd = 0.0_8
! fluxes balance
  DO j=1,jm
...
&       , j, 5, 2) - hndd(i, j+1, 5, 2)
    END DO
  END DO
END SUBROUTINE FLUX_NUM_DNC5_ISO_2D_D_D
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
