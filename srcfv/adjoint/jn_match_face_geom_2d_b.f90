...
!
!  Differentiation of jn_match_face_geom_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs wi
!th!StaticTaping):
...
!===============================================================================
SUBROUTINE JN_MATCH_FACE_GEOM_2D_B(fr, frb, prr, gh1r, gh2r, gh3r, gh4r&
& , imr, jmr, fd, fdb, prd, gh1d, gh2d, gh3d, gh4d, imd, jmd, locr, locd&
& , tr, dimf)
  IMPLICIT NONE
!
...
! Input variables -------------------------------------------------
  ...
  REAL*8, DIMENSION(1-gh1d:imd+1+gh2d, 1-gh3d:jmd+1+gh4d, dimf), INTENT(&
& IN) :: fd
  REAL*8, DIMENSION(1-gh1d:imd+1+gh2d, 1-gh3d:jmd+1+gh4d, dimf) :: fdb
  INTEGER, DIMENSION(2, 2), INTENT(IN) :: prr, prd
  INTEGER, DIMENSION(2), INTENT(IN) :: tr
! Output variables ------------------------------------------------
  REAL*8, DIMENSION(1-gh1r:imr+1+gh2r, 1-gh3r:jmr+1+gh4r, dimf), INTENT(&
& INOUT) :: fr
  REAL*8, DIMENSION(1-gh1r:imr+1+gh2r, 1-gh3r:jmr+1+gh4r, dimf), INTENT(&
& INOUT) :: frb
! Local variables -------------------------------------------------
  ...
  INTEGER :: branch
  INTEGER :: ad_branch
  INTEGER, DIMENSION(2) :: ad_branch0
! -----------------------------------------------------------------
  istep = SIGN(1, tr(1))
  jstep = SIGN(1, tr(2))
  IF (tr(1) .GE. 0.) THEN
...
!
  IF (idir .EQ. 1) THEN
    ad_branch0(direction) = 1
    ind1 => id
    ind2 => jd
  ELSE IF (idir .EQ. 2) THEN
    ad_branch0(direction) = 0
    ind1 => jd
    ind2 => id
  ELSE
    ad_branch0(direction) = 0
  ...
  DO direction=2,1,-1
    ...
    DO j=ad_to0,ad_from0,-jstep
      ...
      DO i=ad_to,ad_from,-istep
        jr = prr(1, 2) + jdd - 1
        fdb(ind1, ind2, dird) = fdb(ind1, ind2, dird) + sgnnx(dird)*frb(&
&         ir, jr, direction)
    ...
    END IF
  END DO
END SUBROUTINE JN_MATCH_FACE_GEOM_2D_B
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
