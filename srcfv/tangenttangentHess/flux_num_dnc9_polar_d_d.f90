...
&       1, 5) + d3*w(i, j+2, 5) - d4*w(i, j+3, 5) + d5*w(i, j+4, 5)
      IF (half*(y0(i, j)+y0(i, j+1)) - ym .GE. 0.) THEN
        abs0 = half*(y0(i, j)+y0(i, j+1)) - ym
      ELSE
        abs0 = -(half*(y0(i, j)+y0(i, j+1))-ym)
      END IF
...
!
      ...
      IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
        abs2dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
        abs2d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
        abs2d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
        abs2 = p(i-1, j) - two*p(i, j) + p(i+1, j)
      ELSE
        abs2dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
        abs2d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
        abs2d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
        abs2 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
      END IF
...
!1st direction
      temp3 = abs2*abs34d/abs34
      temp2 = (abs2d-temp3)/abs34
      k_sensor1dd = (abs2dd-(abs34d*abs2d0+abs2*abs34dd-temp3*abs34d0)/&
&       abs34-temp2*abs34d0)/abs34
      k_sensor1d = temp2
      k_sensor1d0 = (abs2d0-abs2*abs34d0/abs34)/abs34
      k_sensor1 = abs2/abs34
      IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
        abs3dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
        abs3d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
        abs3d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
        abs3 = p(i-2, j) - two*p(i-1, j) + p(i, j)
      ELSE
        abs3dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
        abs3d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
        abs3d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
        abs3 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
      END IF
      ...
      ELSE
        ...
        abs35 = -x13
      END IF
      temp3 = abs3*abs35d/abs35
      temp2 = (abs3d-temp3)/abs35
      k_sensor2dd = (abs3dd-(abs35d*abs3d0+abs3*abs35dd-temp3*abs35d0)/&
&       abs35-temp2*abs35d0)/abs35
      k_sensor2d = temp2
      k_sensor2d0 = (abs3d0-abs3*abs35d0/abs35)/abs35
      k_sensor2 = abs3/abs35
...
!
      ...
      IF (p(i, j-1) - two*p(i, j) + p(i, j+1) .GE. 0.) THEN
        abs4dd = pdd(i, j-1) - two*pdd(i, j) + pdd(i, j+1)
        abs4d = pd(i, j-1) - two*pd(i, j) + pd(i, j+1)
        abs4d0 = pd0(i, j-1) - two*pd0(i, j) + pd0(i, j+1)
        abs4 = p(i, j-1) - two*p(i, j) + p(i, j+1)
      ELSE
        abs4dd = two*pdd(i, j) - pdd(i, j-1) - pdd(i, j+1)
        abs4d = two*pd(i, j) - pd(i, j-1) - pd(i, j+1)
        abs4d0 = two*pd0(i, j) - pd0(i, j-1) - pd0(i, j+1)
        abs4 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
      END IF
...
! 2nd direction
      temp3 = abs4*abs36d/abs36
      temp2 = (abs4d-temp3)/abs36
      k_sensor1dd = (abs4dd-(abs36d*abs4d0+abs4*abs36dd-temp3*abs36d0)/&
&       abs36-temp2*abs36d0)/abs36
      k_sensor1d = temp2
      k_sensor1d0 = (abs4d0-abs4*abs36d0/abs36)/abs36
      k_sensor1 = abs4/abs36
      IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
        abs5dd = pdd(i, j-2) - two*pdd(i, j-1) + pdd(i, j)
        abs5d = pd(i, j-2) - two*pd(i, j-1) + pd(i, j)
        abs5d0 = pd0(i, j-2) - two*pd0(i, j-1) + pd0(i, j)
        abs5 = p(i, j-2) - two*p(i, j-1) + p(i, j)
      ELSE
        abs5dd = two*pdd(i, j-1) - pdd(i, j-2) - pdd(i, j)
        abs5d = two*pd(i, j-1) - pd(i, j-2) - pd(i, j)
        abs5d0 = two*pd0(i, j-1) - pd0(i, j-2) - pd0(i, j)
        abs5 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
      END IF
      ...
      ELSE
        ...
        abs37 = -x15
      END IF
      temp3 = abs5*abs37d/abs37
      temp2 = (abs5d-temp3)/abs37
      k_sensor2dd = (abs5dd-(abs37d*abs5d0+abs5*abs37dd-temp3*abs37d0)/&
&       abs37-temp2*abs37d0)/abs37
      k_sensor2d = temp2
      k_sensor2d0 = (abs5d0-abs5*abs37d0/abs37)/abs37
      k_sensor2 = abs5/abs37
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs8*abs38d/abs38
    temp2 = (abs8d-temp3)/abs38
    k_sensor1dd = (abs8dd-(abs38d*abs8d0+abs8*abs38dd-temp3*abs38d0)/&
&     abs38-temp2*abs38d0)/abs38
    k_sensor1d = temp2
    k_sensor1d0 = (abs8d0-abs8*abs38d0/abs38)/abs38
    k_sensor1 = abs8/abs38
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs39 = -x17
    END IF
    temp3 = abs9*abs39d/abs39
    temp2 = (abs9d-temp3)/abs39
    k_sensor2dd = (abs9dd-(abs39d*abs9d0+abs9*abs39dd-temp3*abs39d0)/&
&     abs39-temp2*abs39d0)/abs39
    k_sensor2d = temp2
    k_sensor2d0 = (abs9d0-abs9*abs39d0/abs39)/abs39
    k_sensor2 = abs9/abs39
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
&     temp3*(divu2d0+vort2d0))/(divu2+vort2+1d-15)
    ...
    arg1 = temp0*temp + 2.5d0
    temp3 = -(TANH(arg1)*TANH(arg1)) + 1.0
    dxm1dd = -(half*(temp3*arg1dd-arg1d*2*TANH(arg1)*(1.0-TANH(arg1)**2)&
&     *arg1d0))
    dxm1d = -(half*(temp3*arg1d))
    dxm1d0 = -(half*(1.0-TANH(arg1)**2)*arg1d0)
    dxm1 = half*(one-TANH(arg1))
...
&     temp3*(divu2d0+vort2d0))/(divu2+vort2+1d-15)
    ...
    arg1 = temp0*temp + 2.5d0
    temp3 = -(TANH(arg1)*TANH(arg1)) + 1.0
    dxm2dd = -(half*(temp3*arg1dd-arg1d*2*TANH(arg1)*(1.0-TANH(arg1)**2)&
&     *arg1d0))
    dxm2d = -(half*(temp3*arg1d))
    dxm2d0 = -(half*(1.0-TANH(arg1)**2)*arg1d0)
    dxm2 = half*(one-TANH(arg1))
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs10*abs40d/abs40
    temp2 = (abs10d-temp3)/abs40
    k_sensor1dd = (abs10dd-(abs40d*abs10d0+abs10*abs40dd-temp3*abs40d0)/&
&     abs40-temp2*abs40d0)/abs40
    k_sensor1d = temp2
    k_sensor1d0 = (abs10d0-abs10*abs40d0/abs40)/abs40
    k_sensor1 = abs10/abs40
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs41 = -x19
    END IF
    temp3 = abs11*abs41d/abs41
    temp2 = (abs11d-temp3)/abs41
    k_sensor2dd = (abs11dd-(abs41d*abs11d0+abs11*abs41dd-temp3*abs41d0)/&
&     abs41-temp2*abs41d0)/abs41
    k_sensor2d = temp2
    k_sensor2d0 = (abs11d0-abs11*abs41d0/abs41)/abs41
    k_sensor2 = abs11/abs41
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs14dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs14d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs14d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs14 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs14dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs14d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs14d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs14 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs14*abs42d/abs42
    temp2 = (abs14d-temp3)/abs42
    k_sensor1dd = (abs14dd-(abs42d*abs14d0+abs14*abs42dd-temp3*abs42d0)/&
&     abs42-temp2*abs42d0)/abs42
    k_sensor1d = temp2
    k_sensor1d0 = (abs14d0-abs14*abs42d0/abs42)/abs42
    k_sensor1 = abs14/abs42
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs15dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs15d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs15d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs15 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs15dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs15d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs15d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs15 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs43 = -x21
    END IF
    temp3 = abs15*abs43d/abs43
    temp2 = (abs15d-temp3)/abs43
    k_sensor2dd = (abs15dd-(abs43d*abs15d0+abs15*abs43dd-temp3*abs43d0)/&
&     abs43-temp2*abs43d0)/abs43
    k_sensor2d = temp2
    k_sensor2d0 = (abs15d0-abs15*abs43d0/abs43)/abs43
    k_sensor2 = abs15/abs43
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i, j-1) - two*p(i, j) + p(i, j+1) .GE. 0.) THEN
      abs16dd = pdd(i, j-1) - two*pdd(i, j) + pdd(i, j+1)
      abs16d = pd(i, j-1) - two*pd(i, j) + pd(i, j+1)
      abs16d0 = pd0(i, j-1) - two*pd0(i, j) + pd0(i, j+1)
      abs16 = p(i, j-1) - two*p(i, j) + p(i, j+1)
    ELSE
      abs16dd = two*pdd(i, j) - pdd(i, j-1) - pdd(i, j+1)
      abs16d = two*pd(i, j) - pd(i, j-1) - pd(i, j+1)
      abs16d0 = two*pd0(i, j) - pd0(i, j-1) - pd0(i, j+1)
      abs16 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs16*abs44d/abs44
    temp2 = (abs16d-temp3)/abs44
    k_sensor1dd = (abs16dd-(abs44d*abs16d0+abs16*abs44dd-temp3*abs44d0)/&
&     abs44-temp2*abs44d0)/abs44
    k_sensor1d = temp2
    k_sensor1d0 = (abs16d0-abs16*abs44d0/abs44)/abs44
    k_sensor1 = abs16/abs44
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
      abs17dd = pdd(i, j-2) - two*pdd(i, j-1) + pdd(i, j)
      abs17d = pd(i, j-2) - two*pd(i, j-1) + pd(i, j)
      abs17d0 = pd0(i, j-2) - two*pd0(i, j-1) + pd0(i, j)
      abs17 = p(i, j-2) - two*p(i, j-1) + p(i, j)
    ELSE
      abs17dd = two*pdd(i, j-1) - pdd(i, j-2) - pdd(i, j)
      abs17d = two*pd(i, j-1) - pd(i, j-2) - pd(i, j)
      abs17d0 = two*pd0(i, j-1) - pd0(i, j-2) - pd0(i, j)
      abs17 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs45 = -x23
    END IF
    temp3 = abs17*abs45d/abs45
    temp2 = (abs17d-temp3)/abs45
    k_sensor2dd = (abs17dd-(abs45d*abs17d0+abs17*abs45dd-temp3*abs45d0)/&
&     abs45-temp2*abs45d0)/abs45
    k_sensor2d = temp2
    k_sensor2d0 = (abs17d0-abs17*abs45d0/abs45)/abs45
    k_sensor2 = abs17/abs45
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs20*abs46d/abs46
    temp2 = (abs20d-temp3)/abs46
    k_sensor1dd = (abs20dd-(abs46d*abs20d0+abs20*abs46dd-temp3*abs46d0)/&
&     abs46-temp2*abs46d0)/abs46
    k_sensor1d = temp2
    k_sensor1d0 = (abs20d0-abs20*abs46d0/abs46)/abs46
    k_sensor1 = abs20/abs46
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
    ...
    ELSE
      ...
      abs47 = -x25
    END IF
    temp3 = abs21*abs47d/abs47
    temp2 = (abs21d-temp3)/abs47
    k_sensor2dd = (abs21dd-(abs47d*abs21d0+abs21*abs47dd-temp3*abs47d0)/&
&     abs47-temp2*abs47d0)/abs47
    k_sensor2d = temp2
    k_sensor2d0 = (abs21d0-abs21*abs47d0/abs47)/abs47
    k_sensor2 = abs21/abs47
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i, j-1) - two*p(i, j) + p(i, j+1) .GE. 0.) THEN
      abs22dd = pdd(i, j-1) - two*pdd(i, j) + pdd(i, j+1)
      abs22d = pd(i, j-1) - two*pd(i, j) + pd(i, j+1)
      abs22d0 = pd0(i, j-1) - two*pd0(i, j) + pd0(i, j+1)
      abs22 = p(i, j-1) - two*p(i, j) + p(i, j+1)
    ELSE
      abs22dd = two*pdd(i, j) - pdd(i, j-1) - pdd(i, j+1)
      abs22d = two*pd(i, j) - pd(i, j-1) - pd(i, j+1)
      abs22d0 = two*pd0(i, j) - pd0(i, j-1) - pd0(i, j+1)
      abs22 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs22*abs48d/abs48
    temp2 = (abs22d-temp3)/abs48
    k_sensor1dd = (abs22dd-(abs48d*abs22d0+abs22*abs48dd-temp3*abs48d0)/&
&     abs48-temp2*abs48d0)/abs48
    k_sensor1d = temp2
    k_sensor1d0 = (abs22d0-abs22*abs48d0/abs48)/abs48
    k_sensor1 = abs22/abs48
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
      abs23dd = pdd(i, j-2) - two*pdd(i, j-1) + pdd(i, j)
      abs23d = pd(i, j-2) - two*pd(i, j-1) + pd(i, j)
      abs23d0 = pd0(i, j-2) - two*pd0(i, j-1) + pd0(i, j)
      abs23 = p(i, j-2) - two*p(i, j-1) + p(i, j)
    ELSE
      abs23dd = two*pdd(i, j-1) - pdd(i, j-2) - pdd(i, j)
      abs23d = two*pd(i, j-1) - pd(i, j-2) - pd(i, j)
      abs23d0 = two*pd0(i, j-1) - pd0(i, j-2) - pd0(i, j)
      abs23 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs49 = -x27
    END IF
    temp3 = abs23*abs49d/abs49
    temp2 = (abs23d-temp3)/abs49
    k_sensor2dd = (abs23dd-(abs49d*abs23d0+abs23*abs49dd-temp3*abs49d0)/&
&     abs49-temp2*abs49d0)/abs49
    k_sensor2d = temp2
    k_sensor2d0 = (abs23d0-abs23*abs49d0/abs49)/abs49
    k_sensor2 = abs23/abs49
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs26dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs26d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs26d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs26 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs26dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs26d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs26d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs26 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs26*abs50d/abs50
    temp2 = (abs26d-temp3)/abs50
    k_sensor1dd = (abs26dd-(abs50d*abs26d0+abs26*abs50dd-temp3*abs50d0)/&
&     abs50-temp2*abs50d0)/abs50
    k_sensor1d = temp2
    k_sensor1d0 = (abs26d0-abs26*abs50d0/abs50)/abs50
    k_sensor1 = abs26/abs50
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs27dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs27d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs27d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs27 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs27dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs27d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs27d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs27 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs51 = -x29
    END IF
    temp3 = abs27*abs51d/abs51
    temp2 = (abs27d-temp3)/abs51
    k_sensor2dd = (abs27dd-(abs51d*abs27d0+abs27*abs51dd-temp3*abs51d0)/&
&     abs51-temp2*abs51d0)/abs51
    k_sensor2d = temp2
    k_sensor2d0 = (abs27d0-abs27*abs51d0/abs51)/abs51
    k_sensor2 = abs27/abs51
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i, j-1) - two*p(i, j) + p(i, j+1) .GE. 0.) THEN
      abs28dd = pdd(i, j-1) - two*pdd(i, j) + pdd(i, j+1)
      abs28d = pd(i, j-1) - two*pd(i, j) + pd(i, j+1)
      abs28d0 = pd0(i, j-1) - two*pd0(i, j) + pd0(i, j+1)
      abs28 = p(i, j-1) - two*p(i, j) + p(i, j+1)
    ELSE
      abs28dd = two*pdd(i, j) - pdd(i, j-1) - pdd(i, j+1)
      abs28d = two*pd(i, j) - pd(i, j-1) - pd(i, j+1)
      abs28d0 = two*pd0(i, j) - pd0(i, j-1) - pd0(i, j+1)
      abs28 = -(p(i, j-1)-two*p(i, j)+p(i, j+1))
    END IF
...
! #include "rhs/spectralradiusRossow_j.F"
! 2nd direction
    temp3 = abs28*abs52d/abs52
    temp2 = (abs28d-temp3)/abs52
    k_sensor1dd = (abs28dd-(abs52d*abs28d0+abs28*abs52dd-temp3*abs52d0)/&
&     abs52-temp2*abs52d0)/abs52
    k_sensor1d = temp2
    k_sensor1d0 = (abs28d0-abs28*abs52d0/abs52)/abs52
    k_sensor1 = abs28/abs52
    IF (p(i, j-2) - two*p(i, j-1) + p(i, j) .GE. 0.) THEN
      abs29dd = pdd(i, j-2) - two*pdd(i, j-1) + pdd(i, j)
      abs29d = pd(i, j-2) - two*pd(i, j-1) + pd(i, j)
      abs29d0 = pd0(i, j-2) - two*pd0(i, j-1) + pd0(i, j)
      abs29 = p(i, j-2) - two*p(i, j-1) + p(i, j)
    ELSE
      abs29dd = two*pdd(i, j-1) - pdd(i, j-2) - pdd(i, j)
      abs29d = two*pd(i, j-1) - pd(i, j-2) - pd(i, j)
      abs29d0 = two*pd0(i, j-1) - pd0(i, j-2) - pd0(i, j)
      abs29 = -(p(i, j-2)-two*p(i, j-1)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs53 = -x31
    END IF
    temp3 = abs29*abs53d/abs53
    temp2 = (abs29d-temp3)/abs53
    k_sensor2dd = (abs29dd-(abs53d*abs29d0+abs29*abs53dd-temp3*abs53d0)/&
&     abs53-temp2*abs53d0)/abs53
    k_sensor2d = temp2
    k_sensor2d0 = (abs29d0-abs29*abs53d0/abs53)/abs53
    k_sensor2 = abs29/abs53
!     ducros1 is done in dissipation_ducros_x
    divudd = gradudd(i, j-1, 1) + gradvdd(i, j-1, 2)
    divud = gradud(i, j-1, 1) + gradvd(i, j-1, 2)
    divud0 = gradud0(i, j-1, 1) + gradvd0(i, j-1, 2)
...
!
    ...
    rspec = ab + sq
    IF (p(i-1, j) - two*p(i, j) + p(i+1, j) .GE. 0.) THEN
      abs31dd = pdd(i-1, j) - two*pdd(i, j) + pdd(i+1, j)
      abs31d = pd(i-1, j) - two*pd(i, j) + pd(i+1, j)
      abs31d0 = pd0(i-1, j) - two*pd0(i, j) + pd0(i+1, j)
      abs31 = p(i-1, j) - two*p(i, j) + p(i+1, j)
    ELSE
      abs31dd = two*pdd(i, j) - pdd(i-1, j) - pdd(i+1, j)
      abs31d = two*pd(i, j) - pd(i-1, j) - pd(i+1, j)
      abs31d0 = two*pd0(i, j) - pd0(i-1, j) - pd0(i+1, j)
      abs31 = -(p(i-1, j)-two*p(i, j)+p(i+1, j))
    END IF
...
! #include "rhs/spectralradiusRossow_i.F"
!1st direction
    temp3 = abs31*abs54d/abs54
    temp2 = (abs31d-temp3)/abs54
    k_sensor1dd = (abs31dd-(abs54d*abs31d0+abs31*abs54dd-temp3*abs54d0)/&
&     abs54-temp2*abs54d0)/abs54
    k_sensor1d = temp2
    k_sensor1d0 = (abs31d0-abs31*abs54d0/abs54)/abs54
    k_sensor1 = abs31/abs54
    IF (p(i-2, j) - two*p(i-1, j) + p(i, j) .GE. 0.) THEN
      abs32dd = pdd(i-2, j) - two*pdd(i-1, j) + pdd(i, j)
      abs32d = pd(i-2, j) - two*pd(i-1, j) + pd(i, j)
      abs32d0 = pd0(i-2, j) - two*pd0(i-1, j) + pd0(i, j)
      abs32 = p(i-2, j) - two*p(i-1, j) + p(i, j)
    ELSE
      abs32dd = two*pdd(i-1, j) - pdd(i-2, j) - pdd(i, j)
      abs32d = two*pd(i-1, j) - pd(i-2, j) - pd(i, j)
      abs32d0 = two*pd0(i-1, j) - pd0(i-2, j) - pd0(i, j)
      abs32 = -(p(i-2, j)-two*p(i-1, j)+p(i, j))
    END IF
    ...
    ELSE
      ...
      abs55 = -x33
    END IF
    temp3 = abs32*abs55d/abs55
    temp2 = (abs32d-temp3)/abs55
    k_sensor2dd = (abs32dd-(abs55d*abs32d0+abs32*abs55dd-temp3*abs55d0)/&
&     abs55-temp2*abs55d0)/abs55
    k_sensor2d = temp2
    k_sensor2d0 = (abs32d0-abs32*abs55d0/abs55)/abs55
    k_sensor2 = abs32/abs55
    divudd = gradudd(i, j, 1) + gradvdd(i, j, 2)
    divud = gradud(i, j, 1) + gradvd(i, j, 2)
    divud0 = gradud0(i, j, 1) + gradvd0(i, j, 2)
    divu = gradu(i, j, 1) + gradv(i, j, 2)
...
&       , j, 1, 2) - hndd(i, j+1, 1, 2)
      residudd(i, j, 2) = hndd(i, j, 2, 1) - hndd(i+1, j, 2, 1) - src2dd&
&       + hndd(i, j, 2, 2) - hndd(i, j+1, 2, 2)
      residudd(i, j, 3) = hndd(i, j, 3, 1) - hndd(i+1, j, 3, 1) - src3dd&
&       + hndd(i, j, 3, 2) - hndd(i, j+1, 3, 2)
      residudd(i, j, 4) = hndd(i, j, 4, 1) - hndd(i+1, j, 4, 1) - src4dd&
&       + hndd(i, j, 4, 2) - hndd(i, j+1, 4, 2)
      residudd(i, j, 5) = hndd(i, j, 5, 1) - hndd(i+1, j, 5, 1) - src5dd&
