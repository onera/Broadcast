...
!
!  Differentiation of bc_no_reflexion_forhessian_2d_d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDea
!dInstrs with!StaticTaping):
!   variations   of useful results: woutd
!   with respect to varying inputs: w
!   RW status of diff variables: w:in woutd:out
!        Generated by TAPENADE     (INRIA, Ecuador team)
!  Tapenade 3.16 (master) -  9 Oct 2020 17:47
!
!  Differentiation of bc_no_reflexion_forhessian_2d in forward (tangent) mode (with options with!SliceDeadControl with!SliceDeadI
...
!==============================================================================
!
SUBROUTINE BC_NO_REFLEXION_FORHESSIAN_2D_D_D(wout, woutd, woutdd, w, wd0&
& , wd, wbd, loc, interf, nx, ny, gam, gh, im, jm, lm)
  IMPLICIT NONE
...
! Local variables -----------------------------------------------------------
  ...
  REAL*8 :: uudd, vvdd, wwdd, roc0dd, p0dd
  ...
  REAL*8 :: usdd, vsdd, wsdd, psdd, utsdd, vtsdd, vnsdd
...
& b0dd
  ...
  REAL*8 :: ro0m1d0, roc0m1d0, rosm1d0, rom1d0
  ...
  REAL*8 :: roud0, rovd0, rowd0, roe1d0
...
! 0-State --------------------------------------------------------------
    ...
    ro0d0 = wd0(i, j, 1)
    ro0 = w(i, j, 1)
    temp0 = one*ro0d/(ro0*ro0)
    ro0m1dd = temp0*2*ro0d0/ro0
    ro0m1d = -temp0
    ro0m1d0 = -(one*ro0d0/ro0**2)
    ro0m1 = one/ro0
    uudd = wd(i, j, 2)*ro0m1d0 + ro0m1d*wd0(i, j, 2) + w(i, j, 2)*&
&     ro0m1dd
    uud = ro0m1*wd(i, j, 2) + w(i, j, 2)*ro0m1d
    uud0 = ro0m1*wd0(i, j, 2) + w(i, j, 2)*ro0m1d0
    uu = w(i, j, 2)*ro0m1
    vvdd = wd(i, j, 3)*ro0m1d0 + ro0m1d*wd0(i, j, 3) + w(i, j, 3)*&
&     ro0m1dd
    vvd = ro0m1*wd(i, j, 3) + w(i, j, 3)*ro0m1d
    vvd0 = ro0m1*wd0(i, j, 3) + w(i, j, 3)*ro0m1d0
    vv = w(i, j, 3)*ro0m1
    wwdd = wd(i, j, 4)*ro0m1d0 + ro0m1d*wd0(i, j, 4) + w(i, j, 4)*&
&     ro0m1dd
    ...
    roe1dd = -(half*(ro0d*tempd+temp0*ro0d0+ro0*(uud*2*uud0+2*uu*uudd+&
&     vvd*2*vvd0+2*vv*vvdd+wwd*2*wwd0+2*ww*wwdd)))
    roe1d = wd(i, j, 5) - half*(ro0d*temp+ro0*temp0)
    ...
    result1d0 = tempd
    result1 = temp
    roc0dd = ro0d*result1d0 + result1d*ro0d0 + ro0*result1dd
...
! sch-State ---------------------------------------------------------------
    ...
    rosd0 = wd0(i, j, 1)
    ros = w(i, j, 1)
    temp0 = one*rosd/(ros*ros)
    rosm1dd = temp0*2*rosd0/ros
    rosm1d = -temp0
    rosm1d0 = -(one*rosd0/ros**2)
    rosm1 = one/ros
    usdd = wd(i, j, 2)*rosm1d0 + rosm1d*wd0(i, j, 2) + w(i, j, 2)*&
&     rosm1dd
    usd = rosm1*wd(i, j, 2) + w(i, j, 2)*rosm1d
    usd0 = rosm1*wd0(i, j, 2) + w(i, j, 2)*rosm1d0
    us = w(i, j, 2)*rosm1
    vsdd = wd(i, j, 3)*rosm1d0 + rosm1d*wd0(i, j, 3) + w(i, j, 3)*&
&     rosm1dd
    vsd = rosm1*wd(i, j, 3) + w(i, j, 3)*rosm1d
    vsd0 = rosm1*wd0(i, j, 3) + w(i, j, 3)*rosm1d0
    vs = w(i, j, 3)*rosm1
    wsdd = wd(i, j, 4)*rosm1d0 + rosm1d*wd0(i, j, 4) + w(i, j, 4)*&
&     rosm1dd
    ...
    psdd = -(gam1*half*(rosd*tempd+temp0*rosd0+ros*(usd*2*usd0+2*us*usdd&
&     +vsd*2*vsd0+2*vs*vsdd+wsd*2*wsd0+2*ws*wsdd)))
    psd = gam1*(wd(i, j, 5)-half*(rosd*temp+ros*temp0))
...
&     amd0)+(ap-am)*roc0m1dd)
    ...
    temp = (p-ps)*(ro0*ro0)
    temp0 = ro0*ro0*(pd0-psd) + 2*ro0d*(p-ps)*ro0
    temp1 = 2*temp*roc0m1
    bsdd = temp0*2*roc0m1*roc0m1d0 + roc0m1**2*((pd0-psd)*2*ro0*ro0d0+&
&     ro0**2*(pd0d-psdd)+ro0d*2*(ro0*(pd1-psd0)+(p-ps)*ro0d0)) + roc0m1d&
&     *(roc0m1*2*tempd+2*temp*roc0m1d0) + temp1*roc0m1dd
    bsd = rosd + roc0m1*roc0m1*temp0 + temp1*roc0m1d
    bsd0 = roc0m1**2*tempd + temp*2*roc0m1*roc0m1d0 + rosd0
    bs = temp*(roc0m1*roc0m1) + ros
    tempd = ro0**2*pd1 + (p-pd)*2*ro0*ro0d0
    temp = (p-pd)*(ro0*ro0)
    temp1 = ro0*ro0*pd0 + 2*ro0d*(p-pd)*ro0
    temp0 = 2*temp*roc0m1
    b0dd = temp1*2*roc0m1*roc0m1d0 + roc0m1**2*(pd0*2*ro0*ro0d0+ro0**2*&
&     pd0d+ro0d*2*(ro0*pd1+(p-pd)*ro0d0)) + roc0m1d*(roc0m1*2*tempd+2*&
&     temp*roc0m1d0) + temp0*roc0m1dd
...
&     row*rowdd))
    DO de=1,gh
      ...
      woutdd(i-de*i0, j-de*j0, 4) = rowdd
! w(i-de*i0,j-de*j0,5) = roe + HALF * rom1 * (rou*rou+rov*rov+row*row)
      woutdd(i-de*i0, j-de*j0, 5) = roe2dd
!extrap o2
! roe2 =  roe
      rod0d = two*rod0d - woutdd(i-da*i0, j-da*j0, 1)
      roudd = two*roudd - woutdd(i-da*i0, j-da*j0, 2)
      rovdd = two*rovdd - woutdd(i-da*i0, j-da*j0, 3)
      rowdd = two*rowdd - woutdd(i-da*i0, j-da*j0, 4)
! roe  =  TWO * roe - roe1
! roe1 =  roe2
! rom1 =  ONE/ro
      roe2dd = two*roe2dd - woutdd(i-da*i0, j-da*j0, 5)
    END DO
  END DO
END SUBROUTINE BC_NO_REFLEXION_FORHESSIAN_2D_D_D
!
