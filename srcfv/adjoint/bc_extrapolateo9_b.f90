...
!
!  Differentiation of bc_extrapolate_o9_2d in reverse (adjoint) mode (with options with!SliceDeadControl with!SliceDeadInstrs wit
!h!StaticTaping):
!   gradient     of useful results: w
!   with respect to varying inputs: w
!   RW status of diff variables: w:in-out
SUBROUTINE BC_EXTRAPOLATE_O9_2D_B(w, wb, loc, interf, im, jm, gh, em)
  IMPLICIT NONE
...
!   write(200,*) loc, 'jmax = ', interf(2,2)
  ...
  DO l=lmax,lmin,-1
    DO de=gh,1,-1
      ...
      wb(i-i0*d1, j-j0*d1, :) = wb(i-i0*d1, j-j0*d1, :) + 9.d0*tmpb
      wb(i-i0*d2, j-j0*d2, :) = wb(i-i0*d2, j-j0*d2, :) - 36.d0*tmpb
      wb(i-i0*d3, j-j0*d3, :) = wb(i-i0*d3, j-j0*d3, :) + 84.d0*tmpb
      wb(i-i0*d5, j-j0*d5, :) = wb(i-i0*d5, j-j0*d5, :) + 126.d0*tmpb
      wb(i-i0*d4, j-j0*d4, :) = wb(i-i0*d4, j-j0*d4, :) - 126.d0*tmpb
      wb(i-i0*d7, j-j0*d7, :) = wb(i-i0*d7, j-j0*d7, :) + 36.d0*tmpb
      wb(i-i0*d6, j-j0*d6, :) = wb(i-i0*d6, j-j0*d6, :) - 84.d0*tmpb
      wb(i-i0*d9, j-j0*d9, :) = wb(i-i0*d9, j-j0*d9, :) + tmpb
      wb(i-i0*d8, j-j0*d8, :) = wb(i-i0*d8, j-j0*d8, :) - 9.d0*tmpb
    END DO
    CALL POPINTEGER4(j)
    CALL POPINTEGER4(i)
  END DO
  CALL POPCONTROL2B(branch)
END SUBROUTINE BC_EXTRAPOLATE_O9_2D_B
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! End of file
